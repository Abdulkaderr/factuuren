 ...
 * [BACKEND-JSON-RESPONSE] [EXAMPLES]
   ...
   ----------------------------------------------------------
        ...
        [dom_classes_delayed]
            //[action options] add|remove
            //Executed after dom_attributes_end - useful for changing classes on newly added DOM content
            $jsondata['dom_classes_delayed'][] = [
                'selector' => '#newly-added-element',
                'action' => 'add',
                'value' => 'some-class-name',
			];
...
function nxAjaxUxRequest(obj, virtualPostArray = {}) {
	...
	NXAJAX.getPayload = function (obj) {
		...
		//dom classes (delayed - executed after dom_attributes_end)
		NXAJAX.payload.dom_classes_delayed = obj.dom_classes_delayed;
	...
	/**-----------------------------------------------------------------------------------------------------------------------------------
	 * [UPDATE - DOM CLASSES (DELAYED)] (if applicable)
	 *
	 * This function updates DOM classes after other DOM manipulations have been completed.
	 * It is executed after NXAJAX.updateDomAttributes('end'), allowing class changes on newly added content.
	 *
	 * [JQUERY]
	 *    $("#foo").addClass('bar');
	 *    $("#foo").removeClass('bar');
	 *
	 * [EXAMPLE DATA SENT]
	 *    ['dom_classes_delayed'][0]['selector'] = '#main-table'       : valid dom selector '.some_class' | '#some_id' | '[input-type=""]'
	 *    ['dom_classes_delayed'][0]['action'] = 'add'                 : add|remove
	 *    ['dom_classes_delayed'][0]['value'] = 'some-class-name'      : new value
	 *
	 *------------------------------------------------------------------------------------------------------------------------------------*/
	NXAJAX.updateDelayedDomClasses = function () {

		//get the payload
		var payload = NXAJAX.payload.dom_classes_delayed;

		//debug
		NXAJAX.log('[ajax] updateDelayedDomClasses() - updating dom classes (delayed) if applicable - [payload]:', payload);
		//update the DOM (id|class|literal)
		if (payload != undefined && typeof payload == 'object') {
			//loop through the payload and update the dom
			$.each(payload, function (index, value) {
	...
	var ajax_request = $.ajax({
		...
		success: function (data) {
			...
			//update delayed DOM classes (for content added during this ajax call)
			NXAJAX.updateDelayedDomClasses();
