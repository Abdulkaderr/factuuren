...
/*----------------------------------------------------------------
 * INLINE DISCOUNTS NAMESPACE
 * All discount-related functions isolated here
 *--------------------------------------------------------------*/
NXINVOICE.DISCOUNT = {

    /**
     * Calculate discount amount for a single line item
     * @param {number} subtotal - Line subtotal (qty * rate)
     * @param {string} type - 'none', 'fixed', or 'percentage'
     * @param {number} value - The discount value
     * @return {number} - Calculated discount amount
     */
    calculateLineDiscount: function(subtotal, type, value) {
        var discount = 0;

        if (type === 'fixed') {
            discount = parseFloat(value) || 0;
        } else if (type === 'percentage') {
            discount = (subtotal * (parseFloat(value) || 0)) / 100;
        }
    ...
    /**
     * Recalculate all line item discounts
     * Updates hidden discount amount fields
     */
    recalculateLineDiscounts: function() {

        NXINVOICE.log("[billing] recalculateLineDiscounts() - calculating discounts for each line item");

        $('.billing-line-item').each(function() {

            var $row = $(this);
            var type = $row.attr('type');

            // Get line item values based on type
            var qty = 0;
            var rate = parseFloat($row.find('.js_item_rate').val()) || 0;
            var subtotal = 0;

            if (type == 'plain') {
                qty = parseFloat($row.find('.js_item_quantity').val()) || 0;
                subtotal = qty * rate;
            } else if (type == 'time') {
                var hours = parseFloat($row.find('.js_item_hours').val()) || 0;
                var minutes = parseFloat($row.find('.js_item_minutes').val()) || 0;
                var minutes_total = 0;
                ...
                var length = parseFloat($row.find('.js_item_length').val()) || 0;
                var width = parseFloat($row.find('.js_item_width').val()) || 0;
                qty = parseFloat($row.find('.js_item_quantity').val()) || 0;
                var area = accounting.toFixed((length * width), 2);
                subtotal = area * rate * qty;
            }

            // Get discount values
            var discountType = $row.find('.js_item_discount_type').val() || 'none';
            var discountValue = parseFloat($row.find('.js_item_discount_value').val()) || 0;

            // Calculate discount
            var discountAmount = NXINVOICE.DISCOUNT.calculateLineDiscount(
                subtotal,
                discountType,
                discountValue
            );
            ...
            // Update hidden field
            $row.find('.js_item_discount_amount').val(discountAmount.toFixed(2));

            NXINVOICE.log("[billing] recalculateLineDiscounts() - discount calculated [type]: (" + discountType + ") [value]: (" + discountValue + ") [amount]: (" + discountAmount + ")");
        });
    }
};
...
NXINVOICE.DOM.itemNewLine = function (data = {}) {
    ...
    var item_default_tax = (data.item_default_tax != null && data.item_default_tax != '') ? data.item_default_tax : '5';
    var item_notes = (data.item_notes != null) ? data.item_notes : '';
    ...
    lineitem.find(".js_item_long_description").val(item_notes);
    ...
    //auto-select default tax rate for product items
    if (item_default_tax != '' && item_default_tax != null) {
        var matching_option = lineitem.find(".js_item_tax option[data-id='" + item_default_tax + "']");
        if (matching_option.length > 0) {
            var tax_value = matching_option.val();
            lineitem.find(".js_item_tax").val(tax_value);
            NXINVOICE.log("[billing] NXINVOICE.DOM.itemNewLine() - auto-selected tax rate ID: " + item_default_tax);
        } else {
            NXINVOICE.log("[billing] NXINVOICE.DOM.itemNewLine() - no matching tax rate found for ID: " + item_default_tax + ", defaulting to empty");
        }
    }

    //select2 (tax)
    lineitem.find(".js_item_tax").select2({
        allowClear: false,
    ...
    lineitem.find(".js_item_discount_type").attr("name", "js_item_discount_type[" + uniqueid + "]");
    lineitem.find(".js_item_discount_value").attr("name", "js_item_discount_value[" + uniqueid + "]");
    lineitem.find(".js_item_discount_amount").attr("name", "js_item_discount_amount[" + uniqueid + "]");
    lineitem.find(".js_item_long_description").attr("name", "js_item_long_description[" + uniqueid + "]");
    ...
    $("#billing-items-container").find(".billing-line-item").each(function () {
        ...
        if (type == 'plain') {
            ...
            if (quantity > 0 && rate > 0) {
                ...
                var line_subtotal = quantity * rate;

                //update net total field (subtotal without discount or tax)
                lineitem.find('.js_item_net_total').val(nxFormatDecimal(line_subtotal));

                //calculate discount (if inline mode)
                var line_discount = 0;
                if ($("#bill_tax_type").val() === 'inline') {
                    line_discount = parseFloat(lineitem.find('.js_item_discount_amount').val()) || 0;
                }

                //calculate amount after discount
                var line_after_discount = line_subtotal - line_discount;

                //calculate tax on post-discount amount
                var line_tax = 0;
                if ($("#bill_tax_type").val() === 'inline') {
                    var tax_data = lineitem.find('.js_item_tax').val();
                    if (tax_data) {
                        var tax_data_split = tax_data.split("|");
                        var tax_rate = parseFloat(tax_data_split[0]) || 0;
                        line_tax = (line_after_discount * tax_rate) / 100;
                    }
                }

                //update tax amount display field (inline mode only)
                if ($("#bill_tax_type").val() === 'inline') {
                    lineitem.find('.js_item_tax_amount').val(nxFormatDecimal(line_tax));
                }

                //calculate final line total
                var linetotal = line_after_discount + line_tax;
                ...
                total.val('');
                lineitem.find('.js_item_net_total').val('');
                //reset tax amount field
                if ($("#bill_tax_type").val() === 'inline') {
                    lineitem.find('.js_item_tax_amount').val('0.00');
                }
            }
        }
        ...
        if (type == 'time') {
            ...
            if ((hours > 0 || minutes > 0) && rate > 0) {
                ...
                //line subtotal
                var line_subtotal = hours_total + minutes_total;
                var line_subtotal = accounting.toFixed(line_subtotal, 2);
        ...
        if (type == 'dimensions') {
            ...
            if ((length > 0 || width > 0) && rate > 0 && quantity > 0) {
                ...
                //line subtotal
                var line_subtotal = area * rate * quantity;
                var line_subtotal = accounting.toFixed(line_subtotal, 2);
...
NXINVOICE.CALC.recalculateLineTaxes = function () {
    ...
    $("#billing-items-container").find(".billing-line-item").each(function () {
        ...
        if (NXINVOICE.is_numeric(line_tax_rate)) {
            ...
            //calculate the tax (in inline mode, line_total already includes tax, so extract it)
            // Formula: tax = line_total × (tax_rate / (100 + tax_rate))
            line_tax = (line_total * line_tax_rate) / (100 + parseFloat(line_tax_rate));
    ...
    $("#items-list-table, #category-items-list-table").find(".items-checkbox").each(function () {
        if ($(this).is(":checked")) {
            ...
            var data = {
                ...
                'item_default_tax': $(this).attr('data-default-tax'),
                'item_notes': $(this).attr('data-item-notes'),
            }
...
NXINVOICE.CALC.reclaculateBill = function (self) {
    ...
    //calculate discounts FIRST if in inline mode
    if ($("#bill_tax_type").val() === 'inline') {
        NXINVOICE.DISCOUNT.recalculateLineDiscounts();
    }

    //recalculate lines (will use discount amounts calculated above)
    ...
    var total_line_discounts = 0;
    var bill_subtotal_before_discounts = 0;
    ...
    $("#billing-items-container").find(".billing-line-item").each(function () {
        ...
        //for inline mode, also calculate subtotal before discounts and total discounts
        if ($("#bill_tax_type").val() === 'inline') {
            var line_discount = Number(lineitem.find(".js_item_discount_amount").val()) || 0;
            total_line_discounts += line_discount;

            //calculate line subtotal before discount
            var type = lineitem.attr('type');
            var rate = Number(lineitem.find(".js_item_rate").val()) || 0;
            var line_pre_discount = 0;

            if (type == 'plain') {
                var qty = Number(lineitem.find(".js_item_quantity").val()) || 0;
                line_pre_discount = qty * rate;
            } else if (type == 'time') {
                var hours = Number(lineitem.find(".js_item_hours").val()) || 0;
                var minutes = Number(lineitem.find(".js_item_minutes").val()) || 0;
                line_pre_discount = (hours * rate) + ((minutes / 60) * rate);
            } else if (type == 'dimensions') {
                var length = Number(lineitem.find(".js_item_length").val()) || 0;
                var width = Number(lineitem.find(".js_item_width").val()) || 0;
                var qty = Number(lineitem.find(".js_item_quantity").val()) || 0;
                var area = length * width;
                line_pre_discount = area * rate * qty;
            }

            bill_subtotal_before_discounts += line_pre_discount;
        }
    });
    ...
    // For inline mode, always use pre-discount subtotal (qty × rate). For summary mode, use bill_subtotal
    if ($("#bill_tax_type").val() === 'inline') {
        $("#billing-subtotal-figure").html(accounting.formatMoney(bill_subtotal_before_discounts));
        NXINVOICE.log("[billing] reclaculateBill() - inline mode - subtotal before discounts: (" + bill_subtotal_before_discounts + ") ");
    } else {
        $("#billing-subtotal-figure").html(accounting.formatMoney(bill_subtotal));
    }

    //(2.1) ----------------- UPDATE LINE DISCOUNTS AND SUBTOTAL AFTER DISCOUNT (INLINE MODE ONLY) --------------------------------
    if ($("#bill_tax_type").val() === 'inline') {
        NXINVOICE.log("[billing] reclaculateBill() - updating line discounts [total_line_discounts]: (" + total_line_discounts + ") ");

        // Always show and update line discounts section in inline mode
        $("#billing-line-discounts-total").html('<span>-' + accounting.formatMoney(total_line_discounts) + '</span>');
        $("#billing-table-section-line-discounts").show();

        // Calculate and update subtotal after discount
        var subtotal_after_discount = bill_subtotal_before_discounts - total_line_discounts;
        $("#billing-subtotal-after-discount-figure").html('<span>' + accounting.formatMoney(subtotal_after_discount) + '</span>');
        $("#billing-table-section-subtotal-after-discount").show();

        NXINVOICE.log("[billing] reclaculateBill() - subtotal after discount: (" + subtotal_after_discount + ") ");
    }
    ...
    if ($("#bill_tax_type").val() == 'inline') {
        ...
        if (NXINVOICE.is_numeric(total_inline_tax) && total_inline_tax > 0) {
            ...
            //update tax figure for inline totals template
            $("#billing-tax-total-figure").html('<span>' + accounting.formatMoney(total_inline_tax) + '</span>');
            ...
            //update tax figure for inline totals template (set to 0)
            $("#billing-tax-total-figure").html('<span>' + accounting.formatMoney(0) + '</span>');
    ...
    // In inline mode, bill_subtotal already includes line taxes, so don't add tax again
    if ($("#bill_tax_type").val() === 'inline') {
        bill_final_amount = bill_subtotal;
        NXINVOICE.log("[billing] reclaculateBill() - inline mode - bill_subtotal already includes tax: (" + bill_final_amount + ")");
    } else {
        // In summary mode, add tax and subtract summary discount
        bill_final_amount = bill_subtotal - bill_discount_amount + bill_tax_total_amount;
        NXINVOICE.log("[billing] reclaculateBill() - summary mode - total before adjustment: (" + bill_final_amount + ")");
    }
    ...
    $("#bill_subtotal_before_discount").val(nxFormatDecimal(bill_subtotal_before_discounts));
