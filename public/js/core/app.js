...
function NXRecordMyTmeModal() {
    ...
    //form validation - simplified for multi-row support
    //main validation happens server-side
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        submitHandler: function (form) {
            //ajax request
...
/** ----------------------------------------------------------
 *   [contracts automation] - settings page
 * -----------------------------------------------------------*/
function NXSettingsContractsAutomation() {
    $("#settings-contracts-automation").validate().destroy();
    $("#settings-contracts-automation").validate({
        rules: {
            settings2_contracts_automation_invoice_due_date: {
                required: function (element) {
                    return $("#settings2_contracts_automation_create_invoice").is(":checked");
                }
            },
        },
        submitHandler: function (form) {
...
/** ----------------------------------------------------------
 *   [contracts automation] - edit modal
 * -----------------------------------------------------------*/
function NXContractEditAutomation() {
    $("#commonModalForm").validate().destroy();
    $("#commonModalForm").validate({
        rules: {
            contract_automation_project_title: {
                required: function (element) {
                    return $("#contract_automation_create_project").is(":checked");
                }
            },
            contract_automation_invoice_due_date: {
                required: function (element) {
                    return $("#contract_automation_create_invoice").is(":checked");
                }
            },
        },
        submitHandler: function (form) {
 ...
 * @description: form validation for invoice status
 * -------------------------------------------------------------------------------------*/
function NXSettingsInvoiceStatus() {


    //change invoice color - update form field
    $(document).on('change', '.invoicestatus_colors', function () {
        if (this.checked) {
            $("#invoicestatus_color").val($(this).val());
        }
    });
    ...
    $("#commonModalForm").validate({
        rules: {
            invoicestatus_title: "required"
        },
        submitHandler: function (form) {
            //set selector color
            $(".invoicestatus_colors").each(function () {
                if (this.checked) {
                    $("#invoicestatus_color").val($(this).val());
                }
            });
