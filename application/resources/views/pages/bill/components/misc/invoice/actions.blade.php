...
<div class="col-md-12  col-lg-5 align-self-center text-right p-b-9 {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        <!--copy invoice url-->
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.invoice_url')) }}"
            class="click-to-copy-url list-actions-button btn btn-page-actions waves-effect waves-dark"
            data-url="{{ url('/invoices/view/'.$bill->bill_uniqueid.'?action=preview') }}">
        ...
        <span class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="listTableAction">
                ...
                <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                    ...
                    data-url="{{ url('/invoices/'.$bill->bill_invoiceid.'/cancel') }}"
                    data-loading-target="commonModalBody"
                    data-modal-title="{{ cleanLang(__('lang.cancel_invoice')) }}"
                    data-action-url="{{ url('/invoices/'.$bill->bill_invoiceid.'/cancel') }}"
                    data-action-method="POST">
                    <i class="sl-icon-ban display-inline-block p-r-5"></i>{{ cleanLang(__('lang.cancel_invoice')) }}</a>
                @endif
                @endif
                <!--restore invoice-->
                @if($bill->bill_status == 20)
                <a class="dropdown-item confirm-action-info"
                    href="javascript:void(0)"
                    data-confirm-title="{{ cleanLang(__('lang.restore_invoice')) }}"
                    data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}"
                    data-url="{{ url('/invoices/'.$bill->bill_invoiceid.'/restore') }}"
                    data-ajax-type="POST">
                    <i class="sl-icon-action-undo display-inline-block p-r-5"></i>{{ cleanLang(__('lang.restore_invoice')) }}</a>
                @endif

                <!--change client-->
                <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form"
                    href="javascript:void(0)" data-toggle="modal" data-target="#actionsModal"
                    data-url="{{ url('/invoices/'.$bill->bill_invoiceid.'/change-client') }}"
                    data-action-url="{{ url('/invoices/'.$bill->bill_invoiceid.'/change-client') }}"
                    data-action-method="POST" data-loading-target="actionsModalBody"
                    data-modal-title="@lang('lang.change_client')">
                    <i class="sl-icon-people display-inline-block p-r-5"></i>@lang('lang.change_client')</a>
