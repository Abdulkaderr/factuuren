...
<tr class="billing-line-item" id="lineitem_{{ $lineitem->lineitem_id ?? '' }}" type="plain">
    ...
    <!--tax amount (inline mode only)-->
    <td class="form-group x-tax-amount bill_col_tax_amount {{ runtimeVisibility('invoice-column-inline-tax', $bill->bill_tax_type) }}">
        <input class="form-control form-control-sm js_item_tax_amount decimal-field" type="number" step="0.01"
            name="js_item_tax_amount[{{ $lineitem->lineitem_id ?? '' }}]"
            value="{{ isset($lineitem) ? number_format($lineitem->lineitem_tax_amount, 2, '.', '') : '0.00' }}" disabled>
    </td>
    <!--total-->
    <td class="form-group x-total bill_col_gross_total" id="bill_col_total">
    ...
    <!--net total-->
    <td class="form-group x-net-total bill_col_net_total">
        <input class="form-control form-control-sm js_item_net_total decimal-field" type="number" step="0.01"
            name="js_item_net_total[{{ $lineitem->lineitem_id ?? '' }}]" value="{{ $lineitem->net_line_total ?? '' }}"
            disabled>
    </td>
    ...
    <!-- unique id (stable identifier preserved across saves)-->
    <input type="hidden" class="js_item_uniqueid"
        name="js_item_uniqueid[{{ $lineitem->lineitem_id ?? '' }}]"
