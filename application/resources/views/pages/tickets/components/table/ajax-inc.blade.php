...
<tr id="ticket_{{ $ticket->ticket_id }}" class="{{ $ticket->pinned_status ?? '' }}">
    ...
    <!--tags-->
    @if(config('visibility.tickets_col_tags'))
    <td class="tickets_col_tags" id="tickets_col_tags_{{ $ticket->ticket_id }}">
        <!--tag-->
        @if(count($ticket->tags ?? []) > 0)
        <span class="label label-outline-default">{{ str_limit($ticket->tags->first()->tag_title, 15) }}</span>
        @else
        <span>---</span>
        @endif
        <!--/#tag-->
        ...
        <!--more tags-->
        @if(count($ticket->tags ?? []) > 1)
        @php $tags = $ticket->tags; @endphp
    ...
    <td class="tickets_col_action actions_column">
        ...
        <span class="list-table-action font-size-inherit">
            ...
            <button type="button" title="{{ cleanLang(__('lang.close_ticket')) }}"
                class="data-toggle-action-tooltip btn btn-outline-warning btn-circle btn-sm confirm-action-info"
                data-confirm-title="{{ cleanLang(__('lang.close_ticket')) }}"
                data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="POST"
                data-url="{{ url('/tickets/'.$ticket->ticket_id.'/close?ref=list') }}">
            ...
            @if(config('visibility.action_buttons_edit'))
            @if($ticket->ticket_status == 2)
            <button type="button" title="{{ cleanLang(__('lang.open_ticket')) }}"
                class="data-toggle-action-tooltip btn btn-outline-info btn-circle btn-sm confirm-action-info"
                data-confirm-title="{{ cleanLang(__('lang.open_ticket')) }}"
                data-confirm-text="{{ cleanLang(__('lang.are_you_sure')) }}" data-ajax-type="POST"
                data-url="{{ url('/tickets/'.$ticket->ticket_id.'/open?ref=list') }}">
                <i class="sl-icon-check"></i>
            </button>
            @endif
            @endif
            <!--open ticket-->
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <div class="dropdown-menu" aria-labelledby="listTableAction">
                    ...
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form"
                        ...
                        data-url="{{ url('/tickets/change-status?id='.$ticket->ticket_id) }}"
                    ...
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                        ...
                        data-url="{{ url('/tickets/'.$ticket->ticket_id.'/edit-tags') }}"
                        data-loading-target="commonModalBody" data-modal-title="@lang('lang.edit_tags')"
                        data-action-url="{{ url('/tickets/'.$ticket->ticket_id.'/edit-tags') }}"
                        data-action-method="POST" data-action-ajax-class="ajax-request"
                        data-action-ajax-loading-target="tickets-td-container">
                        @lang('lang.edit_tags')
                    </a>
                    <!--edit tags-->
                </div>
