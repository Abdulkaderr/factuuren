...
<tr id="estimate_{{ $estimate->bill_estimateid }}" class="{{ $estimate->pinned_status ?? '' }}">
    ...
    <!--invoice id (shown when estimate has been converted to an invoice)-->
    <td class="estimates_col_invoice_id" id="estimates_col_invoice_id_{{ $estimate->bill_estimateid }}">
        @if($estimate->bill_converted_to_invoice == 'yes')
        <a href="{{ url('/invoices/'.$estimate->bill_converted_to_invoice_invoiceid) }}">
            {{ runtimeInvoiceIdFormat($estimate->bill_converted_to_invoice_invoiceid) }}
        </a>
        @else
        ---
        @endif
    ...
    <td class="estimates_col_action actions_column" id="estimates_col_action_{{ $estimate->bill_estimateid }}">
        ...
        <span class="list-table-action dropdown  font-size-inherit">
            ...
            <div class="dropdown-menu" aria-labelledby="listTableAction">
                ...
                <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                    ...
                    data-url="{{ url('/estimates/'.$estimate->bill_estimateid.'/edit-tags') }}"
                    data-loading-target="commonModalBody" data-modal-title="@lang('lang.edit_tags')"
                    data-action-url="{{ url('/estimates/'.$estimate->bill_estimateid.'/edit-tags') }}"
                    data-action-method="POST" data-action-ajax-class="ajax-request"
                    data-action-ajax-loading-target="estimates-td-container">
                    @lang('lang.edit_tags')
                </a>
        ...
        <span class="list-table-action">
            <button type="button" title="{{ cleanLang(__('lang.star_estimate')) }}"
                class="data-toggle-action-tooltip btn btn-outline-default btn-circle btn-sm opacity-4 ajax-request {{ $estimate->is_starred ? 'hidden' : '' }} starred-star-button-{{ $estimate->bill_estimateid }}"
                id="starred-star-button-{{ $estimate->bill_estimateid }}"
                data-url="{{ url('/starred/togglestatus?action=star&resource_type=estimate&resource_id='.$estimate->bill_estimateid) }}"
                data-loading-target="starred-star-button-{{ $estimate->bill_estimateid }}" data-ajax-type="POST"
        ...
        <!--unstar button-->
        <span class="list-table-action">
            <button type="button" title="{{ cleanLang(__('lang.unstar_estimate')) }}"
                class="data-toggle-action-tooltip btn btn-outline-default btn-circle btn-sm ajax-request text-warning {{ !$estimate->is_starred ? 'hidden' : '' }} starred-unstar-button-{{ $estimate->bill_estimateid }}"
                id="starred-unstar-button-{{ $estimate->bill_estimateid }}"
                data-url="{{ url('/starred/togglestatus?action=unstar&resource_type=estimate&resource_id='.$estimate->bill_estimateid) }}"
                data-loading-target="starred-unstar-button-{{ $estimate->bill_estimateid }}" data-ajax-type="POST"
                data-on-start-submit-button="disable">
                <i class="sl-icon-star"></i>
            </button>
        </span>
