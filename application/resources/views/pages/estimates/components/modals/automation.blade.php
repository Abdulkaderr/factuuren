    ...
    <div class="form-group row p-b-10">
        <label class="col-sm-12 text-left control-label col-form-label">@lang('lang.automation')</label>
        <div class="col-sm-12">
...
<!--manual automation section - only visible when triggered manually-->
@if(config('visibility.manual_automation'))
<div class="m-t-30" id="manual-automation-section">

    <!--status notice-->
    @if(!is_numeric($estimate->estimate_automation_log_created_project_id ?? null))
    <div class="alert alert-info">
        @lang('lang.estimate_must_be_accepted_for_automation')
    </div>
    @endif
    ...
    <div class="alert alert-danger">
        @lang('lang.automation_already_run_for_this_estimate')
        <a href="{{ url('projects/' . $estimate->estimate_automation_log_created_project_id) }}" target="_blank">
            @lang('lang.project_id'): #{{ $estimate->estimate_automation_log_created_project_id }}
        </a>
    </div>
    @endif
    ...
    <div class="text-right p-t-10">
        <button type="button"
            class="btn btn-default waves-effect m-r-5"
            data-dismiss="modal">
            @lang('lang.cancel')
        </button>
        @if(!is_numeric($estimate->estimate_automation_log_created_project_id ?? null))
        <button type="button"
            ...
            data-url="{{ urlResource('/estimates/' . ($estimate->bill_estimateid ?? '') . '/manual-automation') }}"
            data-ajax-type="POST"
            data-type="form"
            data-form-id="commonModalBody"
            data-loading-target="commonModalBody"
            data-on-start-submit-button="disable">
            @lang('lang.manually_run_automation')
        </button>
        @endif
    </div>
