...
<tr id="proposal_{{ $proposal->doc_id }}" class="{{ $proposal->pinned_status ?? '' }}">
    ...
    <!--tags-->
    @if(config('visibility.proposals_col_tags'))
    <td class="proposals_col_tags" id="proposals_col_tags_{{ $proposal->doc_id }}">
        <!--tag-->
        @if(count($proposal->tags ?? []) > 0)
        <span class="label label-outline-default">{{ str_limit($proposal->tags->first()->tag_title, 15) }}</span>
        @else
        <span>---</span>
        @endif
        <!--/#tag-->
        ...
        <!--more tags-->
        @if(count($proposal->tags ?? []) > 1)
        @php $tags = $proposal->tags; @endphp
        ...
        <span class="list-table-action font-size-inherit">
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <div class="dropdown-menu" aria-labelledby="listTableAction">
                    ...
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                        href="javascript:void(0)" data-toggle="modal" data-target="#commonModal"
                        data-url="{{ url('/proposals/'.$proposal->doc_id.'/edit-tags') }}"
                        data-loading-target="commonModalBody" data-modal-title="@lang('lang.edit_tags')"
                        data-action-url="{{ url('/proposals/'.$proposal->doc_id.'/edit-tags') }}"
