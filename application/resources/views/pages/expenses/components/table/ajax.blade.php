...
<tr id="expense_{{ $expense->expense_id }}" class="{{ $expense->pinned_status ?? '' }}">
    ...
    @endif
    <!--tags-->
    @if(config('visibility.expenses_col_tags'))
    <td class="expenses_col_tags" id="expenses_col_tags_{{ $expense->expense_id }}">
        <!--tag-->
        @if(count($expense->tags ?? []) > 0)
        <span class="label label-outline-default">{{ str_limit($expense->tags->first()->tag_title, 15) }}</span>
        @else
        <span>---</span>
        @endif
        <!--/#tag-->
        ...
        <!--more tags-->
        @if(count($expense->tags ?? []) > 1)
        @php $tags = $expense->tags; @endphp
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <div class="dropdown-menu" aria-labelledby="listTableAction">
                    ...
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                        href="javascript:void(0)" data-toggle="modal" data-target="#commonModal"
                        data-url="{{ url('/expenses/'.$expense->expense_id.'/edit-tags') }}"
                        data-loading-target="commonModalBody" data-modal-title="@lang('lang.edit_tags')"
                        data-action-url="{{ url('/expenses/'.$expense->expense_id.'/edit-tags') }}"
