...
<tr id="contract_{{ $contract->doc_id }}" class="{{ $contract->pinned_status ?? '' }}">
    ...
    <td class="col_doc_id">
        ...
        <!--automation icon-->
        @if(auth()->user()->is_team && $contract->contract_automation_status == 'enabled')
        <span class="sl-icon-energy text-warning p-l-5" data-toggle="tooltip"
            title="@lang('lang.contract_automation')"></span>
        @endif
    </td>
    ...
    <!--tags-->
    @if(config('visibility.contracts_col_tags'))
    <td class="contracts_col_tags" id="contracts_col_tags_{{ $contract->doc_id }}">
        <!--tag-->
        @if(count($contract->tags ?? []) > 0)
        <span class="label label-outline-default">{{ str_limit($contract->tags->first()->tag_title, 15) }}</span>
        @else
        <span>---</span>
        @endif
        <!--/#tag-->
        ...
        <!--more tags-->
        @if(count($contract->tags ?? []) > 1)
        @php $tags = $contract->tags; @endphp
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <div class="dropdown-menu" aria-labelledby="listTableAction">
                    ...
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                        ...
                        data-url="{{ url('/contracts/'.$contract->doc_id.'/edit-tags') }}"
                        data-loading-target="commonModalBody" data-modal-title="@lang('lang.edit_tags')"
                        data-action-url="{{ url('/contracts/'.$contract->doc_id.'/edit-tags') }}"
                    ...
                    <a class="dropdown-item edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                        href="javascript:void(0)" data-toggle="modal" data-target="#commonModal"
                        data-modal-title="{{ cleanLang(__('lang.automation')) }}"
                        data-url="{{ url('/contracts/'.$contract->doc_id.'/edit-automation') }}"
                        data-action-url="{{ url('/contracts/'.$contract->doc_id.'/update-automation') }}"
                        data-loading-target="commonModalBody" data-action-method="PUT"
                        data-action-ajax-class="js-ajax-ux-request"
                        data-action-ajax-loading-target="contracts-td-container">
                        {{ cleanLang(__('lang.automation')) }}</a>
