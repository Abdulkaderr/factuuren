...
<tr id="invoice_{{ $invoice->bill_invoiceid  }}" class="{{ $invoice->pinned_status ?? '' }}">
    ...
    <!--tableconfig_column_24 [tags]-->
    @if(config('visibility.invoices_col_tags'))
    <td class="invoices_col_tags {{ config('table.tableconfig_column_24') }} tableconfig_column_24">
        <!--tag-->
        @if(count($invoice->tags ?? []) > 0)
        @foreach($invoice->tags->take(1) as $tag)
        ...
        <!--more tags-->
        @if(count($invoice->tags ?? []) > 1)
        @php $tags = $invoice->tags; @endphp
        ...
        <span class="list-table-action font-size-inherit">
            ...
            <span class="list-table-action dropdown font-size-inherit">
                ...
                <div class="dropdown-menu" aria-labelledby="listTableAction">
                    ...
                    <!--invoice url-->
                    <a class="dropdown-item click-to-copy-url" href="javascript:void(0);"
                        data-url="{{ url('/invoices/view/'.$invoice->bill_uniqueid.'?action=preview') }}">@lang('lang.invoice_url')</a>
                    ...
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                        ...
                        data-url="{{ url('/invoices/'.$invoice->bill_invoiceid.'/edit-tags') }}"
                        data-loading-target="commonModalBody"
                        data-modal-title="@lang('lang.edit_tags')"
                        data-action-url="{{ url('/invoices/'.$invoice->bill_invoiceid.'/edit-tags') }}"
                    ...
                    @endif
                    <!--change status-->
                    @if(auth()->user()->role->role_invoices > 1 && $invoice->bill_status != 20)
                    ...
                    <!--restore invoice-->
                    @if(auth()->user()->role->role_invoices > 1 && $invoice->bill_status == 20)
                    <a class="dropdown-item confirm-action-info ajax-request"
                        href="javascript:void(0)"
                        data-confirm-title="@lang('lang.restore_invoice')"
                        data-confirm-text="@lang('lang.are_you_sure')"
                        data-url="{{ url('/invoices/'.$invoice->bill_invoiceid.'/restore') }}"
                        data-ajax-type="POST">
                        @lang('lang.restore_invoice')</a>
                    @endif
                    ...
                    @endif
                    <!--cancel invoice-->
                    @if(auth()->user()->role->role_invoices >= 2 && $invoice->bill_status != 20)
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form edit-add-modal-button"
                        href="javascript:void(0)"
                        data-toggle="modal"
                        data-action-ajax-class="ajax-request"
                        data-target="#commonModal"
                        data-url="{{ url('/invoices/'.$invoice->bill_invoiceid.'/cancel') }}"
                        data-loading-target="commonModalBody"
                        data-modal-title="{{ cleanLang(__('lang.cancel_invoice')) }}"
                        data-action-url="{{ url('/invoices/'.$invoice->bill_invoiceid.'/cancel') }}"
                        data-action-method="POST">
                        {{ cleanLang(__('lang.cancel_invoice')) }}</a>
                    @endif
                    <!--change category-->
                    <a class="dropdown-item actions-modal-button js-ajax-ux-request reset-target-modal-form"
                        href="javascript:void(0)" data-toggle="modal" data-target="#actionsModal"
