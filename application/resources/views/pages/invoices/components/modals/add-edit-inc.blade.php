<div class="row" id="js-trigger-invoices-modal-add-edit" data-payload="{{ $page['section'] ?? '' }}"
    data-due-days-text="{{ $invoice_due_days_text ?? '' }}">
    <div class="col-lg-12">
        ...
        <div class="client-selector">
            ...
            <div class="client-selector-container hidden" id="client-new-container">
                <div class="form-group row">
                    <label
                        class="col-sm-12 col-lg-4 text-left control-label col-form-label">{{ cleanLang(__('lang.company_name')) }}</label>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <input type="text" class="form-control  form-control-sm pickadate" name="bill_date_add_edit"
                    autocomplete="off" value="{{ runtimeDatepickerDate($invoice->bill_date ?? runtimeTodaysDate()) }}">
                <input class="mysql-date" type="hidden" name="bill_date" id="bill_date_add_edit"
                    value="{{ $invoice->bill_date ?? runtimeTodaysDateMySQL() }}">
            </div>
        </div>

        
        <!--module extension point - allows modules to inject form fields-->
        @stack('form_invoice_add_edit_2')

        <!--due date-->
        @if(isset($page['section']) && $page['section'] == 'edit')
        <!--EDIT MODE: Standard datepicker-->
        <div class="form-group row">
            <label
        ...
        @else
        <!--CREATE MODE: Automatic/Manual toggle-->
        <div class="form-group row" id="bill_due_date_payload">
            <label
                class="col-sm-12 col-lg-3 text-left control-label col-form-label required">{{ cleanLang(__('lang.due_date')) }}*</label>
            <div class="col-sm-12 col-lg-6">
                <!--Automatic display field (default)-->
                <div id="bill_due_date_auto_container">
                    <input type="text" class="form-control form-control-sm" id="bill_due_date_display"
                        data-system-default="{{ config('system.settings2_invoices_due_days', 7) }}"
                        data-invoice-due-days="{{ $invoice_due_days ?? config('system.settings2_invoices_due_days', 7) }}"
                        disabled
                        value="Invoice Date + {{ $invoice_due_days ?? config('system.settings2_invoices_due_days', 7) }} days">
                    <input type="hidden" name="bill_due_date" id="bill_due_date" value="">
                </div>
                <!--Manual datepicker field (hidden by default)-->
                <div id="bill_due_date_manual_container" class="hidden">
                    <input type="text" class="form-control form-control-sm pickadate" name="bill_due_date_manually"
                        id="bill_due_date_manual" autocomplete="off">
                    <input class="mysql-date" type="hidden" name="bill_due_date_manually" id="bill_due_date_manually"
                        value="">
                </div>
            ...
            <div class="col-sm-12 col-lg-3">
                ...
                <select class="select2-basic form-control form-control-sm select2-preselected"
                    id="invoice_due_date_method" name="invoice_due_date_method"
                    data-preselected="set_due_date_automatically">
                    <option value="set_due_date_automatically">@lang('lang.set_due_date_automatically')</option>
                    <option value="set_due_date_manually">@lang('lang.set_due_date_manually')</option>
                </select>
            </div>
        </div>
        @endif
        ...
        <!--module extension point - allows modules to inject form fields-->
        @stack('form_invoice_add_edit_5')

        <!--module extension point - allows modules to inject form fields-->
        @stack('form_invoice_add_edit_main')
...
<!--invoice due date multilingual support-->
<script>
    NXLANG.invoice_date_plus = "@lang('lang.invoice_date_plus')";
    NXLANG.day = "@lang('lang.day')";
    NXLANG.days = "@lang('lang.days')";
</script>