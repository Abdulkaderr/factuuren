...
<div class="col-md-12  col-lg-7 p-b-9 align-self-center text-right list-page-actions-containers {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        <!--edit customer dropdown - visible only if the user has customer manage or subscription manage permissions-->
        @if(config('visibility.resource_management') || config('visibility.manage_subscriptions'))
        <div class="btn-group">
            ...
            <div class="dropdown-menu" x-placement="top-start"
                ...
                <!--cancel subscription-->
                @if(config('visibility.has_subscription') && config('visibility.manage_subscriptions'))
                ...
                @if(config('subscription_status') == 'awaiting-payment' && config('visibility.manage_subscriptions'))
                <a class="dropdown-item edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                    href="javascript:void(0)" data-toggle="modal" data-target="#commonModal"
                ...
                @if(in_array(config('subscription_status'), ['active', 'free-trial', 'awaiting-payment']) && config('visibility.manage_subscriptions'))
                <a class="dropdown-item edit-add-modal-button js-ajax-ux-request reset-target-modal-form"
                    href="javascript:void(0);" data-toggle="modal" data-target="#commonModal"
