...
Route::middleware('landlord')->group(function () {
    ...
    Route::group(['prefix' => 'app-admin'], function () {
        ...
        Route::group(['prefix' => 'customers'], function () {
            ...
            Route::get("/{customer}/platform-usage", "Landlord\Customers@showPlatformUsage")->where('customer', '[0-9]+');
            Route::get("/{customer}/login", "Landlord\Customers@LoginAsCustomer")->where('customer', '[0-9]+');
            Route::get("/{customer}/error-logs", "Landlord\Customers@errorLogs")->where('customer', '[0-9]+');
            Route::get("/{customer}/error-logs/{log}", "Landlord\Customers@showErrorLog")->where('customer', '[0-9]+')->where('log', '[0-9]+');
            Route::delete("/{customer}/error-logs", "Landlord\Customers@deleteErrorLogs")->where('customer', '[0-9]+');
        });
        ...
        //FRONTEND
        Route::group(['prefix' => 'frontend', 'middleware' => ['App\Http\Middleware\Landlord\AdminOnly']], function () {
        ...
        //TEAM
        Route::resource('team', 'Landlord\Team')->middleware(['App\Http\Middleware\Landlord\AdminOnly']);

        //SETTINGS
        Route::group(['prefix' => 'settings', 'middleware' => ['App\Http\Middleware\Landlord\AdminOnly']], function () {
            ...
            Route::get("/signup-form", "Landlord\Settings\SignupForm@show");
            Route::post("/signup-form", "Landlord\Settings\SignupForm@update")->middleware(['demoModeCheck']);

            //roles
            Route::get("/roles", "Landlord\Settings\Roles@show");
            Route::post("/roles", "Landlord\Settings\Roles@update")->middleware(['demoModeCheck']);
            ...
            //maintenance mode
            Route::get("/maintenance", "Landlord\Settings\Maintenance@show");
            Route::post("/maintenance", "Landlord\Settings\Maintenance@update")->middleware(['demoModeCheck']);

            // Execute SQL Queries on Tenants
            Route::get("/execute-queries", "Landlord\Settings\ExecuteQueries@index");
            Route::get("/execute-queries/create", "Landlord\Settings\ExecuteQueries@create");
            Route::post("/execute-queries", "Landlord\Settings\ExecuteQueries@store")->middleware(['demoModeCheck']);
            Route::delete("/execute-queries/{id}", "Landlord\Settings\ExecuteQueries@destroy")->middleware(['demoModeCheck']);
            Route::get("/execute-queries/{id}/status", "Landlord\Settings\ExecuteQueries@updateStatus")->middleware(['demoModeCheck']);
            Route::get("/execute-queries/{id}/logs", "Landlord\Settings\ExecuteQueries@showLogs");
        });
        ...
        //PREFERENCES
        Route::post("/preferences/tables", "Landlord\Preferences@updateTableConfig");
