...
function runtimeDatepickerDate($date = '') {
    if ($date != '') {
        ...
        try {
            $formatted_date = \Carbon\Carbon::parse($date)->format($date_format);
        } catch (Exception $e) {
            $formatted_date = '';
        }
...
/**
 * return formatted refund id (e.g. #000001)
 * @param numeric $refund_id
 * @return string formatted id or ---
 */
function runtimeRefundIdFormat($refund_id = '') {
    if (is_numeric($refund_id)) {
        return '#' . str_pad($refund_id, 6, '0', STR_PAD_LEFT);
    } else {
        return '---';
    }
}
...
function runtimeVisibility($type = 'invoice-recurring-icon', $value = '', $value2 = '') {
    ...
    //invoice coluns - inline discount
    if ($type == 'invoice-column-inline-discount') {
        return ($value == 'inline') ? '' : 'hidden';
    }
    ...
    //contract automation icon
    if ($type == 'contract-automation-icon') {
        return ($value == 'disabled') ? 'hidden' : '';
    }
}
...
/**
 * various element visibity for contracts automation
 * @param string $value
 * @return string hidden|null
 */
function contractAutomationVisibility($value = '') {
...
/**
 * Format a number by removing unnecessary decimal zeros
 * Returns whole numbers without decimal point, preserves actual decimals
 *
 * @param mixed $number The number to format
 * @return string|float The formatted number
 */
function runtimeDecimalFloat($number) {
    ...
    // Return as float with decimals preserved
    return $float_value;
}

/**
 * bootstrap class, based on value
 * @param string value the status of the invoice
 * @param string type lable|background
 * @return string bootstrap label class
 */
function runtimeInvoiceStatusColors($value = '', $type = 'label') {
    ...
    //get the css value from config
    foreach (config('system.invoice_statuses') as $key => $css) {
        if ($value == $key) {
            $colour = $css;
        }
    }
...
/**
 * invoice custom status title
 * @param string value the status of the invoice
 * @return string status title
 */
function runtimeInvoiceStatusTitle($value = '') {

    //default title
    $status_title = '';

    //get the css value from config
    foreach (config('system.invoice_custom_statuses') as $key => $title) {
        if ($value == $key) {
            $status_title = $title;
        }
    }
...
/**
 * replace {place_holder} variable in $bank_details with actual data
 *
 * @return bool
 */
function parseBankDetails($bank_details = '', $bill = '') {

    //some data to replace
    $invoice_id = $bill->bill_invoiceid;
    $invoice_id_formatted = runtimeInvoiceIdFormat($invoice_id);
    $invoice_total = runtimeMoneyFormat($bill->bill_final_amount);
    $balance_due = runtimeMoneyFormat($bill->invoice_balance ?? $bill->bill_final_amount);
    $client_company_name = $bill->client_company_name ?? '';
    $client_id = $bill->bill_clientid ?? '';
    $due_date = runtimeDate($bill->bill_due_date ?? '');
    $project_title = $bill->project_title ?? '';
    $company_name = config('system.settings_company_name');

    //check if created by user or system
    $creator_name = trim(($bill->first_name ?? '') . ' ' . ($bill->last_name ?? ''));
    $invoice_created_by = !empty($creator_name) ? $creator_name : cleanLang(__('lang.system'));

    //replace placeholders with actual values
    $bank_details = str_replace('{invoice_id}', $invoice_id, $bank_details);
    $bank_details = str_replace('{invoice_id_formatted}', $invoice_id_formatted, $bank_details);
    $bank_details = str_replace('{invoice_total}', $invoice_total, $bank_details);
    $bank_details = str_replace('{balance_due}', $balance_due, $bank_details);
    $bank_details = str_replace('{client_company_name}', $client_company_name, $bank_details);
    $bank_details = str_replace('{client_id}', $client_id, $bank_details);
    $bank_details = str_replace('{due_date}', $due_date, $bank_details);
    $bank_details = str_replace('{project_title}', $project_title, $bank_details);
    $bank_details = str_replace('{company_name}', $company_name, $bank_details);
    $bank_details = str_replace('{invoice_created_by}', $invoice_created_by, $bank_details);

    return $bank_details;
}

/**
 * find all css classes set in the array and create a single string
 *
 * @return string
 */
function setPageCSS($data = '') {
...
/**
 * Return the distraction free mode body class name based on the user's current preference.
 * Used to set the class on the <body> element on page load.
 * @return string 'kanban_card_distraction_free_mode' or ''
 */
function runtimeDistractionFreeBodyClass() {

    if (!auth()->check()) {
        return '';
    }

    $pref = auth()->user()->pref_view_kanban_card_distraction_free ?? 'no';

    return ($pref == 'yes') ? 'kanban_card_distraction_free_mode' : '';
}

/**
 * Check the authenticated user's distraction free mode preference and return
 * the 'hidden' CSS class for the appropriate button type
 * @param string $button_type 'enable' or 'disable'
 * @return string 'hidden' or ''
 */
function distractionFreeModeCheck($button_type = 'enable') {
    ...
    //enable button: hide when distraction free mode is active
    if ($button_type == 'enable') {
        return ($pref == 'yes') ? 'hidden' : '';
    }

    //disable button: hide when distraction free mode is not active
    if ($button_type == 'disable') {
        return ($pref == 'no') ? 'hidden' : '';
    }
...
/**
 * Returns 'active' if the current page was reached by clicking a stats panel
 * and the given identifier matches the active_panel value in the request.
 * @param  string $panel  the human-readable panel identifier (e.g. 'open', 'closed')
 * @return string
 */
function runtimeActiveStatsPanel($panel) {
    if (request('source') == 'stats_panels' && request('active_panel') == $panel) {
        return 'active';
    }
    return '';
}