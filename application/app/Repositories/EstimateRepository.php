...
class EstimateRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        if ($data['apply_filters']) {
            ...
            //filter: converted to invoice (yes|no)
            if (request()->filled('filter_bill_converted_to_invoice')) {
                $estimates->where('bill_converted_to_invoice', request('filter_bill_converted_to_invoice'));
            }
    ...
    public function createProposalEstimate($id) {
        ...
        $estimate->bill_uniqueid = str_unique();
        $estimate->bill_tax_type = config('system.settings2_invoices_default_mode') ?? 'summary';
    ...
    public function update($id) {
        ...
        $estimate->bill_subtotal_before_discount = request('bill_subtotal_before_discount');
        $estimate->bill_notes = request('bill_notes');
        $estimate->bill_categoryid = request('bill_categoryid');
        $estimate->bill_terms = request('bill_terms');
    ...
    public function refreshEstimate($estimate) {
        ...
        if ($estimate->bill_tax_type == 'inline') {
            ...
            foreach ($estimate->lineitems as $lineitem) {
                $line_subtotal = 0;
            ...
            $estimate->bill_subtotal_before_discount = $subtotal_before_discount;
        }
        ...
        if ($estimate->bill_tax_type == 'inline') {
            ...
            $estimate->bill_discount_amount = $total_discount;
        }
        ...
        if (is_array(request('js_item_description'))) {
            foreach (request('js_item_description') as $key => $description) {
                ...
                if (request('js_item_type')[$key] == 'plain') {
                    ...
                    $line = [
                        'lineitem_uniqueid' => request('js_item_uniqueid')[$key] ?? '',
    ...
    public function convertEstimateToInvoice($id) {
        ...
        foreach ($lines as $line) {
            ...
            $lineitem->lineitem_uniqueid = str_unique();
