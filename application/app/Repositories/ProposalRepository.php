...
class ProposalRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        $proposals->leftJoin('projects', 'projects.project_id', '=', 'proposals.doc_project_id');
        ...
        if (in_array(request('sortorder'), array('desc', 'asc')) && request('orderby') != '') {
            ...
            if (Schema::hasColumn('proposals', request('orderby'))) {
                $proposals->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
    ...
    /**
     * Return the proposal body with all placeholder variables replaced with their real values.
     * Handles {pricing_table}, {pricing_total}, and all standard/custom field variables.
     * @param object $proposal the proposal model
     * @return string the fully rendered proposal body
     */
    public function generateRenderedBody($proposal) {

        //get the full joined proposal document (needed for replaceVariables field access)
        if (!$document = $this->search($proposal->doc_id)->first()) {
            return '';
        }

        //start with the raw body
        $body = $proposal->doc_body;

        //default
        $has_estimate = false;

        //render the pricing table if an estimate exists for this proposal
        if ($estimate = \App\Models\Estimate::Where('bill_proposalid', $proposal->doc_id)->Where('bill_estimate_type', 'document')->first()) {
            ...
            if ($payload = $this->estimategenerator->generate($estimate->bill_estimateid)) {
                ...
                $body = str_replace('{pricing_total}', runtimeMoneyFormat($bill->bill_final_amount), $body);
            }
        }
        ...
        if (!$has_estimate) {
            ...
            $body = str_replace('{pricing_total}', '', $body);
        }

        //replace all standard and custom field variables
        $body = $this->contractrepo->replaceVariables($body, $document);
