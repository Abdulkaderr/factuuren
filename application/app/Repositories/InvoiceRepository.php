...
class InvoiceRepository {
    ...
    public function search($id = '', $data = array()) {
        ...
        //[scope] - user roles - limit for own scope
        if (request()->filled('scope_own_invoices') && request('scope_own_invoices') === true) {
        ...
        if ($data['apply_filters']) {
            ...
            if (request('filter_invoice_exclude_status') == 1) {
                $invoices->whereNotIn('bill_status', [1]);
            }

            //filter: tags
            if (is_array(request('filter_tags')) && !empty(array_filter(request('filter_tags')))) {
                $invoices->whereHas('tags', function ($query) {
        ...
        //search: various invoice columns and relationships (where first, then wherehas)
        if (request()->filled('search_query') || request()->filled('query')) {
            $invoices->where(function ($query) {
                ...
                $query->orWhereHas('lineitems', function ($q) {
                    $q->where('lineitem_description', 'LIKE', '%' . request('search_query') . '%')
                        ->orWhere('lineitem_long_description', 'LIKE', '%' . request('search_query') . '%');
                });
            });
        ...
        //stats - sum paid invoices
        if (isset($data['stats']) && $data['stats'] == 'sum-paid') {
            return $invoices->get()->sum('bill_final_amount');
        }
    ...
    public function refreshInvoice($invoice) {
        ...
        //invoice status for none draft invoices (1 = draft, 2 = due, 3 = overdue, 4 = part_paid, 5 = paid) - skip for invoices with manually set status
        if ($invoice->bill_status_trigger != 'manual') {
            if ($invoice->bill_status != 1) {
                ...
                //invoice is overdue
                if ($invoice->bill_status == 2 || $invoice->bill_status == 4) {
        ...
        //recalculate bill_subtotal_before_discount from line items (for inline tax mode)
        if ($invoice->bill_tax_type == 'inline') {
            ...
            foreach ($invoice->lineitems as $lineitem) {
                ...
                } elseif ($lineitem->lineitem_type == 'time') {
                    $total_hours = $lineitem->lineitem_time_hours + ($lineitem->lineitem_time_minutes / 60);
                    $line_subtotal = $total_hours * $lineitem->lineitem_rate;
                } elseif ($lineitem->lineitem_type == 'dimensions') {
                    $area = $lineitem->lineitem_dimensions_length * $lineitem->lineitem_dimensions_width;
                    $line_subtotal = $area * $lineitem->lineitem_rate * $lineitem->lineitem_quantity;
                }
                $subtotal_before_discount += $line_subtotal;
            }
            $invoice->bill_subtotal_before_discount = $subtotal_before_discount;
        }

        //sum all line item discounts and update the invoice
        if ($invoice->bill_tax_type == 'inline') {
            $total_discount = 0;
            foreach ($invoice->lineitems as $lineitem) {
                $total_discount += $lineitem->lineitem_discount_amount;
            }
            $invoice->bill_discount_amount = $total_discount;
        }
    ...
    public function updateInvoice($id) {
        ...
        $invoice->bill_subtotal_before_discount = request('bill_subtotal_before_discount');
    ...
    public function saveLineItems($bill_invoiceid = '') {
        ...
        //collect line item ids for discount processing
        $line_ids = [];
        ...
        if (is_array(request('js_item_description'))) {
            foreach (request('js_item_description') as $key => $description) {
                ...
                if (request('js_item_type')[$key] == 'plain') {
                    ...
                    //calculate discount amount for inline mode
                    $discount_type = 'none';
                    $discount_value = 0;
                    $discount_amount = 0;
                    if ($invoice->bill_tax_type == 'inline') {
                        $discount_type = request('js_item_discount_type')[$key] ?? 'none';
                        $discount_value = request('js_item_discount_value')[$key] ?? 0;
                        $line_subtotal = request('js_item_quantity')[$key] * request('js_item_rate')[$key];
                    ...
                    $line = [
                        ...
                        'lineitem_long_description' => request('js_item_long_description')[$key] ?? null,
                        ...
                        'lineitem_discount_type' => $discount_type,
                        'lineitem_discount_value' => $discount_value,
                        'lineitem_discount_amount' => $discount_amount,
                    ];
                    $lineitem_id = $this->lineitemrepo->create($line);

                    //collect line item id
                    $line_ids[] = $lineitem_id;
                ...
                if (request('js_item_type')[$key] == 'time') {
                    ...
                    if ($invoice->bill_tax_type == 'inline') {
                        $discount_type = request('js_item_discount_type')[$key] ?? 'none';
                        $discount_value = request('js_item_discount_value')[$key] ?? 0;
                        $hours_total = request('js_item_hours')[$key] * request('js_item_rate')[$key];
                        $minutes_total = (request('js_item_minutes')[$key] / 60) * request('js_item_rate')[$key];
                        $line_subtotal = $hours_total + $minutes_total;
                        if ($discount_type == 'fixed') {
                            $discount_amount = floatval($discount_value);
                        } elseif ($discount_type == 'percentage') {
                            $discount_amount = ($line_subtotal * floatval($discount_value)) / 100;
                ...
                if (request('js_item_type')[$key] == 'dimensions') {
                    ...
                    if ($invoice->bill_tax_type == 'inline') {
                        $discount_type = request('js_item_discount_type')[$key] ?? 'none';
                        $discount_value = request('js_item_discount_value')[$key] ?? 0;
                        $line_subtotal = $area * request('js_item_rate')[$key] * request('js_item_quantity')[$key];
                        if ($discount_type == 'fixed') {
                            $discount_amount = floatval($discount_value);
                        } elseif ($discount_type == 'percentage') {
                            $discount_amount = ($line_subtotal * floatval($discount_value)) / 100;
