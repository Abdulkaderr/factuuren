...
class ContractRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        $contracts->selectRaw("(SELECT email
                                      FROM users
                                      WHERE clientid = contracts.doc_client_id AND account_owner = 'yes' LIMIT 1)
                                      AS client_email");
    ...
    public function visibilitySignatures($contract = '', $view = '') {
        ...
        if ($contract->doc_signed_status == 'signed') {
            ...
            //show signature
            if (!auth()->check() || (auth()->check() && auth()->user()->clientid == $contract->doc_client_id) || (auth()->check() && auth()->user()->is_team)) {
    ...
    /**
     * replace standard variables and custom variables
     *
     * @param string $final_document the document content with variables
     * @param object $document the contract/proposal document model
     * @return string the parsed document with all variables replaced
     */
    public function replaceVariables($final_document, $document) {

        //replace standard variables for proposals
        ...
        //replace standard variables for contracts
        if ($document->doc_type == 'contract') {
            $final_document = str_replace('{company_name}', config('system.settings_company_name'), $final_document);
            $final_document = str_replace('{contract_id}', runtimeContractIdFormat($document->doc_id), $final_document);
        ...
        //replace client name variables (handle both lead and client types)
        if ($document->docresource_type == 'lead') {
            ...
            $final_document = str_replace('{client_email}', $document->lead_email ?? '', $final_document);
        } else {
            $final_document = str_replace('{client_company_name}', $document->client_company_name ?? '', $final_document);
            ...
            $final_document = str_replace('{client_email}', $document->client_email ?? '', $final_document);
        }

        //replace custom field variables (only for client-type contracts)
        if ($document->docresource_type == 'client' && $document->doc_type == 'contract') {

            //get enabled custom fields for clients
            $customfields = \App\Models\CustomField::Where('customfields_type', 'clients')
                ->where('customfields_status', 'enabled')
                ->get();

            //replace each custom field variable
            foreach ($customfields as $field) {
                ...
                if($field->customfields_datatype == 'date'){
                    $field_value = runtimeDate($field_value);
                }

                $final_document = str_replace('{' . $field_name . '}', $field_value, $final_document);
            }
        }
    ...
    /**
     * Return the contract body with all placeholder variables replaced with their real values.
     * Handles {pricing_table}, {pricing_table_total}, and all standard/custom field variables.
     * @param object $contract the contract model
     * @return string the fully rendered contract body
     */
    public function generateRenderedBody($contract) {

        //get the full joined contract document (needed for replaceVariables field access)
        if (!$document = $this->search($contract->doc_id)->first()) {
            return '';
        }

        //start with the raw body
        $body = $contract->doc_body;

        //default
        $has_estimate = false;

        //render the pricing table if an estimate exists for this contract
        if ($estimate = \App\Models\Estimate::Where('bill_contractid', $contract->doc_id)->Where('bill_estimate_type', 'document')->first()) {

            request()->merge(['generate_estimate_mode' => 'document']);

            if ($payload = $this->estimategenerator->generate($estimate->bill_estimateid)) {
                ...
                $body = str_replace('{pricing_table}', $rendered_estimate, $body);
                $body = str_replace('{pricing_table_total}', runtimeMoneyFormat($bill->bill_final_amount), $body);
            }
        }

        //no estimate - clear the placeholder tokens
        if (!$has_estimate) {
            $body = str_replace('{pricing_table}', '', $body);
            $body = str_replace('{pricing_table_total}', '', $body);
        }

        //replace all standard and custom field variables
        $body = $this->replaceVariables($body, $document);
