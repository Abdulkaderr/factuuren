...
class ProposalAutomationRepository
{
    ...
    public function createProject($proposal)
    {
        ...
        //use proposal body as project description (with all variables rendered)
        if ($proposal->proposal_automation_project_description == 'yes') {
            $project->project_description = $this->proposalrepo->generateRenderedBody($proposal);
            $project->save();
        }
    ...
    public function createTasks($project, $proposal)
    {
        ...
        if ($items = \App\Models\Lineitem::Where('lineitemresource_type', 'estimate')->where('lineitemresource_id', $estimate->bill_estimateid)->get()) {
            ...
            foreach ($items as $item) {
                ...
                //note: count() is checked separately first to avoid operator precedence issue
                //where $product_tasks would be assigned the boolean result instead of the collection
