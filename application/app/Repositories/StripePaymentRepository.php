...
class StripePaymentRepository {
    ...
    public function onetimePayment($data = []) {
        ...
        if (!$customer = $this->getCustomer($data['payee_id'])) {
            abort(409, __('lang.error_request_could_not_be_completed'));
        }
    ...
    public function getCustomer($user_id = '') {
        ...
        if ($user->gateways_stripe_customer_id != '') {
            //get the customer
            try {
                $customer = \Stripe\Customer::retrieve($user->gateways_stripe_customer_id);
                return $customer;
            } catch (exception $e) {
        ...
        try {
            ...
            Log::error("error retrieving customer from stripe", ['process' => '[StripePaymentRepository]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'user_id' => $user_id, 'error_message' => $e->getMessage()]);
        }
