    ...
    public function configureDB($customer = [], $package = [], $auth_key = '') {
        ...
        if ($new_customer = \Spatie\Multitenancy\Models\Tenant::On('landlord')->Where('tenant_id', $tenant_id)->first()) {
            try {
                ...
                Log::info("importing sql for customer id ($tenant_id) - completed", ['process' => '[create-tenant-database]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'project_id' => 1]);
                return true;
            ...
            Log::critical("error importing sql file failed. Tenant could not be found. ID ($tenant_id)", ['process' => '[create-tenant-database]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'tenant_id' => $tenant_id]);
            return false;
        }
