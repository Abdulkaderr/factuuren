...
class EstimateAutomationRepository {
    ...
    public function createTasks($project, $estimate) {
        ...
        if ($items = \App\Models\Lineitem::Where('lineitemresource_type', 'estimate')->where('lineitemresource_id', $estimate->bill_estimateid)->get()) {
            ...
            foreach ($items as $item) {
                ...
                if (is_numeric($item->lineitem_linked_product_id) && \App\Models\ProductTask::Where('product_task_itemid', $item->lineitem_linked_product_id)->count() > 0) {
                    $product_tasks = \App\Models\ProductTask::Where('product_task_itemid', $item->lineitem_linked_product_id)->get();
                    Log::info("found (" . count($product_tasks) . ") product based tasks. for line item ($item->lineitem_id) will now create tasks using them.", ['process' => '[estimate-automation]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'project_id' => $project->project_id]);
                    ...
                    Log::info("no product based tasks were found for line item ($item->lineitem_id). will now create tasks using the lineitem, title", ['process' => '[estimate-automation]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'project_id' => $project->project_id]);
    ...
    public function assignTaskUsers($task, $project, $obj, $type) {
        ...
        //extended_mode must be false here - automation runs without an authenticated user,
        //so auth()->id() returns null and would produce invalid SQL in the extended subqueries
        $tasks = $this->taskrepo->search($task->task_id, ['apply_filters' => false, 'extended_mode' => false]);
        $task = $tasks->first();
