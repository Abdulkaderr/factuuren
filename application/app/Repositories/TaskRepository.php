    ...
    public function search($id = '', $data = []) {
        ...
        if ($data['extended_mode']) {
            ...
            $tasks->selectRaw('(SELECT COUNT(*)
                                      ...
                                      AND checklistresource_id = tasks.task_id
                                      AND checklist_status = "pending")
                                      AS pending_checklists_count');
        }
        ...
        if (request()->filled('scope_own_tasks') && request('scope_own_tasks') === true) {
        ...
        if ($data['apply_filters']) {
            ...
            //filter: has pending checklists
            if (request()->filled('filter_task_pending_checklists')) {
                if (request('filter_task_pending_checklists') == 'yes') {
                    $tasks->whereHas('checklists', function ($query) {
                        $query->where('checklist_status', 'pending');
                    });
                } elseif (request('filter_task_pending_checklists') == 'no') {
                    $tasks->whereDoesntHave('checklists', function ($query) {
                        $query->where('checklist_status', 'pending');
                    });
                }
            }
         ...
         * SEARCH BLOCK
         * Resived: 28 Feb 2026
         * Accepts: request('search_query') or request('query')
         * The search term is resolved from whichever parameter is present, with
         * search_query taking priority. All conditions are grouped inside a single
         * where() closure so they are wrapped in parentheses and cannot interfere
         * with other WHERE clauses on the query.
         *
         * IMPORTANT - use orWhereIn with raw subqueries, NOT orWhereHas:
         * Using orWhereHas() on polymorphic relationships inside a grouped where()
         * closure can produce incorrect SQL - the type constraint may be dropped or
         * the OR can escape the parenthesis group, causing unrelated rows to match.
         * orWhereIn() with an explicit subquery against the raw table is safe and
         * fully predictable.
         *
         * Searchable fields:
         *   - task_id              (exact match)
         *   - task_title           (partial match)
         *   - tags.tag_title       (partial match, via tags table)
         *   - assigned user        (first_name or last_name exact match, via tasks_assigned + users)
         *   - comments.comment_text (partial match, task-level comments only)
         *   - checklists.checklist_text (partial match)
         *   - checklist comments.comment_text (partial match, comments belonging to checklists of this task)
         */
        if (request()->filled('search_query') || request()->filled('query')) {
            $search = request()->filled('search_query') ? request('search_query') : request('query');
            $tasks->where(function ($query) use ($search) {
                $query->Where('task_id', '=', $search);
                $query->orWhere('task_title', 'LIKE', '%' . $search . '%');
                $query->orWhereIn('task_id', function ($q) use ($search) {
                    $q->select('tagresource_id')->from('tags')
                        ->where('tagresource_type', 'task')
                        ->where('tag_title', 'LIKE', '%' . $search . '%');
                });
                $query->orWhereIn('task_id', function ($q) use ($search) {
                    $q->select('tasksassigned_taskid')->from('tasks_assigned')
                        ->join('users', 'users.id', '=', 'tasks_assigned.tasksassigned_userid')
                        ->where(function ($q2) use ($search) {
                            $q2->where('users.first_name', '=', $search)
                                ->orWhere('users.last_name', '=', $search);
                        });
                });
                $query->orWhereIn('task_id', function ($q) use ($search) {
                    $q->select('commentresource_id')->from('comments')
                        ->where('commentresource_type', 'task')
                        ->where('comment_text', 'LIKE', '%' . $search . '%');
                });
                $query->orWhereIn('task_id', function ($q) use ($search) {
                    $q->select('checklistresource_id')->from('checklists')
                        ->where('checklistresource_type', 'task')
                        ->where(function ($q2) use ($search) {
                            $q2->where('checklist_text', 'LIKE', '%' . $search . '%')
                                ->orWhereIn('checklist_id', function ($q3) use ($search) {
                                    $q3->select('commentresource_id')->from('comments')
 ...
 */
    ...
    /**
     * Get various task stats applying any user role scope limitations.
     *
    ...
    public function stats() {
        ...
        foreach (config('task_statuses') as $status) {
            ...
            //status
            $tasks->where('task_status', $status->taskstatus_id);
