...
class ImapTicketsFetchCron {
    ...
    public function fetchEmails() {
        ...
        Log::info("IMAP FETCHING -  cronjob for fetching support tickets via imap email - started", ['process' => '[imap-ticket-fetch-cron]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);

        //reset categories stuck in 'fetching' status for more than 1 hour
        $reset_count = \App\Models\Category::where('category_type', 'ticket')
            ->where('category_meta_4', 'enabled')
            ->where('category_meta_14', 'processing')
            ->where('category_meta_22', '<', now()->subHour())
            ->update(['category_meta_14' => '']);

        if ($reset_count > 0) {
            Log::info("IMAP FETCHING -  reset ($reset_count) categories that were stuck in [processing] mode", ['process' => '[imap-ticket-fetch-cron]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
        }
        ...
        if (!$category = \App\Models\Category::where('category_type', 'ticket')
            ...
            Log::info("IMAP FETCHING -  no iamp enabled ticket category found - will now exit", ['process' => '[imap-ticket-fetch-cron]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return;
        }
        ...
        Log::info("IMAP FETCHING -  cronjob for fetching support tickets via imap email - ended", ['process' => '[imap-ticket-fetch-cron]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
