...
class TaskAssignment extends Mailable {
    ...
    public function build() {
        ...
        //when no authenticated user exists (e.g. guest acceptance), fall back to the primary admin
        $assigned_by = auth()->check() ? auth()->user() : \App\Models\User::where('id', 1)->first();

        //set template variables
        $payload += [
            'first_name' => $this->user->first_name,
            'last_name' => $this->user->last_name,
            'assigned_by_first_name' => $assigned_by->first_name ?? '',
