...
/** --------------------------------------------------------------------------------
 * This middleware class handles filter processing for tickets
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class Filtering {
    ...
    public function handle($request, Closure $next) {
        ...
        //filter handling
        if (request()->routeIs('tickets.search')) {
    ...
    private function saveFilter() {
        ...
        if (!$filter = \App\Models\Filter::where('filter_userid', auth()->id())
            ->where('filter_type', 'tickets')->first()) {
            $filter = new \App\Models\Filter();
            $filter->filter_userid = auth()->id();
            $filter->filter_type = 'tickets';
     ...
     * the status filter and source params in the redirect), otherwise returns null.
     * @return string|null
     */
