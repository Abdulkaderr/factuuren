...
/** --------------------------------------------------------------------------------
 * This middleware class handles filter processing for contracts
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class Filtering {
    ...
    public function handle($request, Closure $next) {
        ...
        if (request()->filled('clear-filter') && request('clear-filter') == 'yes') {
            ...
            return redirect('/contracts' . $query);
        }

        //filter handling
        if (request()->routeIs('contracts.search')) {
    ...
    private function saveFilter() {
        ...
        if (!$filter = \App\Models\Filter::where('filter_userid', auth()->id())
            ->where('filter_type', 'contracts')->first()) {
            $filter = new \App\Models\Filter();
            $filter->filter_userid = auth()->id();
            $filter->filter_type = 'contracts';
