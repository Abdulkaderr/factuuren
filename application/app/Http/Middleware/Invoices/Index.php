...
class Index {
    ...
    public function handle($request, Closure $next) {
        ...
        if (auth()->user()->is_client) {
            if (auth()->user()->is_client_owner) {
                ...
                request()->merge([
                    'filter_invoice_exclude_status' => 1,
        ...
        if (auth()->user()->is_team) {
            if (auth()->user()->role->role_invoices >= 1) {
                //limited scope - filter for invoices created by this user
                if (auth()->user()->role->role_invoices_scope == 'own') {
                    request()->merge([
                        'scope_own_invoices' => true,
                    ]);
                }
                return $next($request);
            }
    ...
    private function tableConfig() {
        ...
        if (!$table = \App\Models\TableConfig::Where('tableconfig_userid', auth()->id())->Where('tableconfig_table_name', 'invoices')->first()) {
            ...
            $table->tableconfig_column_4 = 'displayed'; //due
            $table->tableconfig_column_5 = (auth()->user()->is_team) ? 'displayed' : 'hidden'; //company
            $table->tableconfig_column_6 = 'hidden'; //user
            $table->tableconfig_column_7 = (auth()->user()->is_team) ? 'displayed' : 'hidden'; //created by
            $table->tableconfig_column_8 = 'hidden'; //project id
            $table->tableconfig_column_9 = 'hidden'; //project title
            $table->tableconfig_column_10 = 'hidden'; //tax
            $table->tableconfig_column_11 = 'hidden'; //discount type % or fixed or ---
            $table->tableconfig_column_12 = 'hidden'; //discount amount
            ...
            $table->tableconfig_column_20 = 'hidden'; //payments
            $table->tableconfig_column_21 = 'displayed'; //amount
            $table->tableconfig_column_22 = 'hidden'; //balance
            $table->tableconfig_column_23 = 'displayed'; //status
            $table->tableconfig_column_24 = 'hidden'; //tags
            $table->save();
        }
        ...
        if (auth()->user()->role->role_invoices >= 1) {
            if (auth()->user()->is_team) {
                config([
                    ...
                    'visibility.invoices_col_tags' => true,
                ]);
            }
            if (auth()->user()->is_client) {
                config([
