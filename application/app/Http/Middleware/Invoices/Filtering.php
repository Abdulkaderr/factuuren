...
/** --------------------------------------------------------------------------------
 * This middleware class handles filter processing for invoices
 *
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class Filtering {
    ...
    public function handle($request, Closure $next) {
        ...
        if (request()->filled('clear-filter') && request('clear-filter') == 'yes') {
            ...
            return redirect('/invoices' . $query);
        }

        //filter handling
        if (request()->routeIs('invoices.search')) {
    ...
    private function saveFilter() {
        ...
        if (!$filter = \App\Models\Filter::where('filter_userid', auth()->id())
            ->where('filter_type', 'invoices')->first()) {
            $filter = new \App\Models\Filter();
            $filter->filter_userid = auth()->id();
            $filter->filter_type = 'invoices';
