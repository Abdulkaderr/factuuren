...
class General {
    ...
    private function lastSeen() {
        ...
        //[MT] - Track tenant activity in landlord database
        $this->tenantActivityTracking();
    ...
    /**
     * Track tenant activity in landlord database (multi-tenant only)
     * Uses cache throttling to prevent excessive database updates
     * @return void
     */
    private function tenantActivityTracking() {

        //only for multi-tenant instances
        if (!env('MT_TPYE')) {
            return;
        }
        ...
        //only for team members (not clients)
        ...
        //cache key for throttling updates (one update per tenant every 5 minutes)
        $cache_key = 'tenant-activity-' . $tenant_id;

        //check if we recently updated (throttle to reduce database writes)
        if (Cache::has($cache_key)) {
            return;
        }

        //update tenant last seen and activity status in landlord database
        \App\Models\Landlord\Tenant::on('landlord')->where('tenant_id', $tenant_id)
            ->update([
                'tenant_tracking_activity_last_seen' => now(),
                'tenant_activity_status' => 'active',
            ]);

        //set cache to throttle next update (5 minutes)
        Cache::put($cache_key, true, \Carbon\Carbon::now()->addMinutes(5));
    }
