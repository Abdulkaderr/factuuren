...
use App\Events\SaaS\Frontend\CustomerSignedUp;
...
class Signup extends Controller {
    ...
    public function createAccount(CreateAccount $request, CreateTenantRepository $createtenantrepo) {
        ...
        //save country if enabled in signup form settings
        if (config('system.settings_signup_form_country') != 'not-included') {
            $customer->tenant_country = request('country');
        }

        //save telephone if enabled in signup form settings
        if (config('system.settings_signup_form_telephone') != 'not-included') {
            $customer->tenant_telephone = request('telephone');
        }
        ...
        //fire customer signed up event
        event(new CustomerSignedUp(request(), ['customer' => $customer]));
