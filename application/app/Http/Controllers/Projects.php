...
use App\Http\Responses\Projects\EditTagsResponse;
...
class Projects extends Controller {
    ...
    public function __construct(
        ...
        //Filtering
        $this->middleware('projectsMiddlewareFiltering')->only([
            'index',
        ]);
    ...
    public function store(
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectStoring(request()));
        ...
        event(new \App\Events\Projects\ProjectStored(request(), $project, $payload));
    ...
    public function update(ProjectValidation $request, ProjectAssignedRepository $assignedrepo, ProjectManagerRepository $managerrepo, $id) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectUpdating(request(), $project));
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectUpdated(request(), $project, $payload));
    ...
    public function destroy(DestroyRepository $destroyrepo) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectDeleting(request(), $project_ids));
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectDeleted(request(), $allrows, $payload));
    ...
    public function stopAllTimers(TimerRepository $timerepo, $id) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectTimersStopping(request(), $id));

        //stop all running timers for this project
        $data = [
            'timer_projectid' => $id,
        ];
        $stopped_timers = $timerepo->stopRunningTimers($data);
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectTimersStopped(request(), $id, $stopped_timers));
    ...
    public function archive($id) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectArchiving(request(), $project));
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectArchived(request(), $project, $payload));
    ...
    public function activate($id) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectActivating(request(), $project));
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectActivated(request(), $project, $payload));
    ...
    public function bulkArchive() {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectBulkArchiving(request(), $project_ids));

        //update projects using whereIn
        $allrows = array();
        $archived_projects = collect();
        foreach (request('ids') as $project_id => $value) {
            if ($value == 'on') {
                //get project and update status
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectBulkArchived(request(), $archived_projects, $payload));
    ...
    public function bulkRestore() {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectBulkRestoring(request(), $project_ids));

        //update projects using whereIn
        $allrows = array();
        $restored_projects = collect();
        foreach (request('ids') as $project_id => $value) {
            if ($value == 'on') {
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectBulkRestored(request(), $restored_projects, $payload));
    ...
    public function bulkChangeProgressUpdate() {
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectBulkProgressChanged(request(), $updated_project_ids, $payload));
    ...
    public function changeStatusUpdate(ProjectAutomationRepository $automationrepo) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectStatusChanging(request(), $project, request('project_status')));
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectStatusChanged(request(), $project, $old_status, $payload));
    ...
    public function updateDescription() {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectDescriptionUpdating(request(), $project));
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectDescriptionUpdated(request(), $project, $payload));
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectCategoryChanging(request(), $project_ids, request('category')));
        ...
        //get updated projects collection and category for event
        $updated_projects = collect();
        foreach ($allrows as $projects) {
            $updated_projects->push($projects->first());
        }
        $category = \App\Models\Category::Where('category_id', request('category'))->first();
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectCategoryChanged(request(), $updated_projects, $category, $payload));
    ...
    public function assignedUsersUpdate(ProjectAssignedRepository $assignedrepo, $id) {
        ...
        //currently assigned
        $currently_assigned = $project->assigned->pluck('id')->toArray();
        $new_assignments = request()->filled('assigned') ? request('assigned') : [];

        //fire pre-action event
        event(new \App\Events\Projects\ProjectAssignmentChanging(request(), $project, $currently_assigned, $new_assignments));
        ...
        //get removed assignments for event
        $removed_assignments = array_diff($currently_assigned, $new_assignments);
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectAssignmentChanged(request(), $project, $newly_signed_users, $removed_assignments, $payload));
    ...
    public function changeProgressUpdate($id) {
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectProgressChanged(request(), $project, $old_progress, $payload));
    ...
    public function BulkChangeStatusUpdate(ProjectAutomationRepository $automationrepo) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectBulkStatusChanging(request(), $project_ids, request('project_status'), request()->filled('skip_notifications')));
        ...
        foreach (request('ids') as $project_id => $value) {
            if ($value == 'on') {
                ...
                //record status change for event
                $status_changes[] = [
                    'project_id' => $project_id,
                    'old_status' => $old_status,
                    'new_status' => request('project_status'),
                ];
                ...
                //add to updated projects collection
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectBulkStatusChanged(request(), $updated_projects, $status_changes, $payload));
    ...
    public function BulkchangeAssignedUpdate(ProjectAssignedRepository $assignedrepo) {
        ...
        //collect updated projects and assignment changes for event
        $updated_projects = collect();
        $assignment_changes = [];
        foreach ($allrows as $projects) {
            $project = $projects->first();
            ...
            $assignment_changes[] = [
                'project_id' => $project->project_id,
                'newly_assigned' => request()->filled('assigned') ? request('assigned') : [],
            ];
        }
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectBulkAssignmentChanged(request(), $updated_projects, $assignment_changes, $payload));
    ...
    public function togglePinning(PinnedRepository $pinrepo, $id) {
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectPinToggled(request(), $id, $status, auth()->id()));
    ...
    public function bulkStopTimers(TimerRepository $timerepo) {
        ...
        //fire pre-action event
        event(new \App\Events\Projects\ProjectBulkTimersStopping(request(), $project_ids));
        ...
        foreach (request('ids') as $project_id => $value) {
            if ($value == 'on') {
                ...
                $timer_result = $timerepo->stopRunningTimers($data);
                $stopped_timers[$project_id] = $timer_result;
        ...
        //fire post-action event
        event(new \App\Events\Projects\ProjectBulkTimersStopped(request(), $project_ids, $stopped_timers, $payload));
    ...
    public function editTags($id) {
        ...
        //get tags
        $tags_resource = $this->tagrepo->getByResource('project', $id);
        $tags_system = $this->tagrepo->getByType('project');
    ...
    public function updateTags($id) {
        ...
        //get all tags (type: project) - for filter panel
        $tags = $this->tagrepo->getByType('project');
    ...
    private function statsWidget($data = array()) {
        ...
        //when a stats panel is clicked, the URL carries filter params (e.g. project status) that scope the list results.
        ...
        $count_all = $this->projectrepo->search('', ['stats' => 'count-all', 'apply_filters' => $apply_filters]);
        $count_in_progress = $this->projectrepo->search('', ['stats' => 'count-in-progress', 'apply_filters' => $apply_filters]);
        $count_on_hold = $this->projectrepo->search('', ['stats' => 'count-on-hold', 'apply_filters' => $apply_filters]);
        $count_completed = $this->projectrepo->search('', ['stats' => 'count-completed', 'apply_filters' => $apply_filters]);
        ...
        $stats = [
            [
                ...
                'color' => 'bg-info',
                'active_panel' => 'all',
                'url' => url('projects?clear-filter=yes&source=stats_panels&active_panel=all'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-primary',
                'active_panel' => 'in_progress',
                'url' => url('projects?clear-filter=yes&filter_project_status[]=in_progress&source=stats_panels&active_panel=in_progress'),
            ],
            [
                'value' => $count_on_hold,
                ...
                'percentage' => '100%',
                'color' => 'bg-danger',
                'active_panel' => 'on_hold',
                'url' => url('projects?clear-filter=yes&filter_project_status[]=on_hold&source=stats_panels&active_panel=on_hold'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-success',
                'active_panel' => 'completed',
                'url' => url('projects?clear-filter=yes&filter_project_status[]=completed&source=stats_panels&active_panel=completed'),
            ],
        ];
