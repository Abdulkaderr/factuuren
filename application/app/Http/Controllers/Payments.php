...
use App\Http\Responses\Payments\RefundActionsResponse;
...
use App\Http\Responses\Payments\EditTagsResponse;
...
class Payments extends Controller {
    ...
    public function index(ProjectRepository $projectrepo) {
        ...
        $tags = $this->tagrepo->getByType('payment');
    ...
    public function store(PaymentStoreUpdate $request, InvoiceRepository $invoicerepo) {
        ...
        //add tags
        $this->tagrepo->add('payment', $payment_id);
    ...
    public function destroy() {
        ...
        foreach (request('ids') as $id => $value) {
            ...
            if ($value == 'on') {
                //get the payment
                $payment = \App\Models\Payment::Where('payment_id', $id)->first();
                //cascade delete linked refund if one exists
                if ($refund = \App\Models\Refund::where('refund_paymentid', $payment->payment_id)->first()) {
                    $refund->delete();
                }
                //delete payment
                $payment->delete();
    ...
    public function thankYou() {
        ...
        //team member redirect to home (only if authenticated)
        if (auth()->check() && auth()->user()->type == 'team') {
    ...
    public function thankYouTap(TapPaymentRepository $taprepo) {
        ...
        if (!request()->filled('session_id') || !request()->filled('tap_id')) {
            //redirect to home if authenticated, otherwise show error
            if (auth()->check()) {
                return redirect('/home');
            }
            abort(404);
    ...
    /**
     * Show the form to create a refund for a payment (opened from payments list)
     * @param int $id payment id
     * @return \Illuminate\Http\Response
     */
    public function refundCreate($id) {
        ...
        $html = view('pages/payments/components/modals/refund-create', compact('payment'))->render();
    ...
    /**
     * Create a refund for a payment from the payments list page
     * @param int $id payment id
     * @return \Illuminate\Http\Response
     */
    public function refundStore($id) {
        ...
        return new RefundActionsResponse($payload);
    }

    /**
     * Delete the refund linked to a payment (from the payments list page)
     * @param int $id payment id
     * @return \Illuminate\Http\Response
     */
    public function refundDestroy($id) {
    ...
    /**
     * Update the tags for the specified payment
     * @param int $id payment id
     * @return \Illuminate\Http\Response
     */
    public function updateTags($id) {
        ...
        $this->tagrepo->delete('payment', $id);

        //add new tags
        $this->tagrepo->add('payment', $id);

        //get updated payment
        $payments = $this->paymentrepo->search($id, ['apply_filters' => false]);
    ...
    private function statsWidget($data = array()) {
        ...
        //when a stats panel is clicked, the URL carries filter params (e.g. date range) that scope the list results.
        ...
        $count_all = $this->paymentrepo->search('', ['stats' => 'count-all', 'apply_filters' => $apply_filters]);
        $sum_all = $this->paymentrepo->search('', ['stats' => 'sum-all', 'apply_filters' => $apply_filters]);
        $sum_today = $this->paymentrepo->search('', ['stats' => 'sum-today', 'apply_filters' => $apply_filters]);
        $sum_this_month = $this->paymentrepo->search('', ['stats' => 'sum-this-month', 'apply_filters' => $apply_filters]);
        $sum_this_year = $this->paymentrepo->search('', ['stats' => 'sum-this-year', 'apply_filters' => $apply_filters]);
        ...
        $stats = [
            [
                ...
                'color' => 'bg-info',
                'active_panel' => 'all',
                'url' => url('payments?clear-filter=yes&source=stats_panels&active_panel=all'),
            ],
            [
                ...
                'color' => 'bg-primary',
                'active_panel' => 'today',
                'url' => url('payments?clear-filter=yes&filter_payment_date_start=' . date('Y-m-d') . '&filter_payment_date_end=' . date('Y-m-d') . '&source=stats_panels&active_panel=today'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-success',
                'active_panel' => 'this_month',
                'url' => url('payments?clear-filter=yes&filter_payment_date_start=' . date('Y-m-01') . '&filter_payment_date_end=' . date('Y-m-t') . '&source=stats_panels&active_panel=this_month'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-default',
                'active_panel' => 'this_year',
                'url' => url('payments?clear-filter=yes&filter_payment_date_start=' . date('Y-01-01') . '&filter_payment_date_end=' . date('Y-12-31') . '&source=stats_panels&active_panel=this_year'),
            ],
        ];
