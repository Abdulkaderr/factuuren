...
use App\Events\Tasks\TaskStoring;
...
use App\Http\Requests\Estimates\StoreTasksRequest;
...
use App\Http\Responses\Estimates\CreateTasksResponse;
...
use App\Http\Responses\Estimates\EditTagsResponse;
use App\Http\Responses\Estimates\IndexResponse;
use App\Http\Responses\Estimates\ManualAutomationResponse;
...
use App\Http\Responses\Estimates\StoreTasksResponse;
...
class Estimates extends Controller {
    ...
    public function __construct(
        ...
        $this->middleware('App\Http\Middleware\Estimates\StoreTasks')->only([
            'storeTasks',
        ]);
     ...
     */
    /**
     * Show the create tasks modal for the given estimate
     * @param int $id estimate id
     * @return \Illuminate\Http\Response
     */
    public function createTasks($id) {
        ...
        $lineitems = \App\Models\Lineitem::Where('lineitemresource_type', 'estimate')
            ->Where('lineitemresource_id', $id)
            ->orderBy('lineitem_position', 'asc')
            ->get();

        //get all task statuses
        $statuses = \App\Models\TaskStatus::orderBy('taskstatus_position', 'asc')->get();

        //get all team users for assignment dropdown
        $team_users = \App\Models\User::Where('type', 'team')
            ->orderBy('first_name', 'asc')
            ->get();

        //get line item ids that have already been converted to tasks for this project
        $converted_lineitem_ids = [];
        $milestones = collect();
        if (is_numeric($estimate->bill_projectid)) {
            $converted_lineitem_ids = \App\Models\Task::Where('task_mapping_type', 'lineitem')
                ->Where('task_projectid', $estimate->bill_projectid)
                ->pluck('task_mapping_uniqueid')
                ->toArray();
            $milestones = \App\Models\Milestone::Where('milestone_projectid', $estimate->bill_projectid)
                ->orderBy('milestone_position', 'asc')
                ->get();
        }

        //count how many of this estimate's line items are already converted (drives the warning notice)
        $estimate_lineitem_ids = $lineitems->pluck('lineitem_uniqueid')->toArray();
        $disabled_lineitem_count = count(array_intersect($estimate_lineitem_ids, $converted_lineitem_ids));
        ...
        $payload = [
            ...
            'has_project' => is_numeric($estimate->bill_projectid),
            'converted_lineitem_ids' => $converted_lineitem_ids,
            'disabled_lineitem_count' => $disabled_lineitem_count,
        ];

        return new CreateTasksResponse($payload);
    }

    /**
     * Create tasks from selected estimate line items
     * @param TaskAssignedRepository $assignedrepo
     * @param int $id estimate id
     * @return \Illuminate\Http\Response
     */
    public function storeTasks(StoreTasksRequest $request, TaskAssignedRepository $assignedrepo, $id) {
        ...
        //estimate must be attached to a project
        if (!is_numeric($estimate->bill_projectid)) {
            abort(409, __('lang.create_tasks_requires_project'));
        }

        //verify the submitted milestone belongs to this project
        if (!$milestone = \App\Models\Milestone::Where('milestone_id', request('milestone_id'))
            ->Where('milestone_projectid', $estimate->bill_projectid)->first()) {
            abort(409, __('lang.error_request_could_not_be_completed'));
        }

        //get default priority
        $priority = \App\Models\TaskPriority::Where('taskpriority_system_default', 'yes')->first();
        $priority_id = $priority ? $priority->taskpriority_id : 1;

        //create one task per selected line item
        foreach (request('selected_lineitems') as $lineitem_uniqueid) {

            //get and verify the line item belongs to this estimate
            if (!$lineitem = \App\Models\Lineitem::Where('lineitem_uniqueid', $lineitem_uniqueid)
                ->Where('lineitemresource_type', 'estimate')
                ->Where('lineitemresource_id', $id)->first()) {
                continue;
            }

            //calculate position (recalculated each iteration to prevent duplicates)
            if ($last = \App\Models\Task::orderBy('task_position', 'desc')->first()) {
            ...
            //fire storing event
            event(new TaskStoring(request()));

            //create the task
            $task = new \App\Models\Task();
            $task->task_uniqueid = str_unique();
            $task->task_creatorid = auth()->id();
            $task->task_projectid = $estimate->bill_projectid;
            $task->task_clientid = $estimate->bill_clientid;
            $task->task_milestoneid = $milestone->milestone_id;
            $task->task_title = $lineitem->lineitem_description;
            $task->task_description = $lineitem->lineitem_long_description;
            $task->task_status = request('task_status');
            $task->task_priority = $priority_id;
            $task->task_position = $position;
            $task->taskresource_type = 'project';
            $task->task_mapping_type = 'lineitem';
            $task->task_mapping_uniqueid = $lineitem->lineitem_uniqueid;
            $task->save();

            //assign team members if any were selected (reads request('assigned') internally)
            $assignedrepo->add($task->task_id, '');

            //fire stored event
            event(new TaskStored(request(), $task, ['task' => $task]));
        }

        return new StoreTasksResponse([]);
    }
    ...
    /**
     * Show the manual automation form in a modal
     * @param int $id estimate id
     * @return \Illuminate\Http\Response
     */
    public function manualAutomation($id) {
        ...
        return new ManualAutomationResponse($payload);
    }

    /**
     * Save automation settings and trigger the automation manually
     * @param  \App\Http\Requests\Estimates\EstimateUpdateAutomation  $request
     * @param  \App\Repositories\EstimateAutomationRepository  $automationrepo
     * @param  int  $id estimate id
     * @return \Illuminate\Http\Response
     */
    public function manualAutomationAction(EstimateUpdateAutomation $request, EstimateAutomationRepository $automationrepo, $id) {
        ...
        //validation: estimate must be in accepted status
        if ($estimate->bill_status !== 'accepted') {
            abort(409, __('lang.estimate_must_be_accepted_for_automation'));
        }
        ...
        //reset and re-save assigned users
        \App\Models\AutomationAssigned::Where('automationassigned_resource_type', 'estimate')
            ->Where('automationassigned_resource_id', $id)
            ->delete();
        ...
        //re-fetch estimate with updated data then run automation
        $estimate = \App\Models\Estimate::Where('bill_estimateid', $id)->first();
        $automationrepo->process($estimate);
    ...
    public function editTags($id) {
        ...
        //get tags
        $tags_resource = $this->tagrepo->getByResource('estimate', $id);
        $tags_system = $this->tagrepo->getByType('estimate');
    ...
    public function updateTags($id) {
        ...
        //get all tags (type: estimate) - for filter panel
        $tags = $this->tagrepo->getByType('estimate');
 ...
 */
    ...
    /**
     * Bulk disable automation on selected estimates
     * @return \Illuminate\Http\Response
     */
    public function bulkDisableAutomation() {
        ...
        foreach (request('ids') as $bill_estimateid => $value) {
            if ($value == 'on') {
                ...
                //disable automation and reset all automation settings
                $estimate->estimate_automation_status = 'disabled';
                $estimate->estimate_automation_create_project = 'no';
                $estimate->estimate_automation_project_title = '';
                $estimate->estimate_automation_project_status = 'not_started';
                $estimate->estimate_automation_create_tasks = 'no';
                ...
                //remove assigned automation users for this estimate
                \App\Models\AutomationAssigned::Where('automationassigned_resource_type', 'estimate')
                    ->Where('automationassigned_resource_id', $bill_estimateid)
                    ->delete();
    ...
    private function statsWidget($data = array()) {
        ...
        //when a stats panel is clicked, URL filter params must not scope the stats queries,
        //or every panel would reflect the filtered totals instead of its own independent value.
        //disabling apply_filters here ensures stats always show global figures.
        $apply_filters = (request('source') == 'stats_panels') ? false : true;

        //stats
        $count_new = $this->estimaterepo->search('', ['stats' => 'count-new', 'apply_filters' => $apply_filters]);
        $count_accepted = $this->estimaterepo->search('', ['stats' => 'count-accepted', 'apply_filters' => $apply_filters]);
        $count_declined = $this->estimaterepo->search('', ['stats' => 'count-declined', 'apply_filters' => $apply_filters]);
        $count_expired = $this->estimaterepo->search('', ['stats' => 'count-expired', 'apply_filters' => $apply_filters]);

        $sum_new = $this->estimaterepo->search('', ['stats' => 'sum-new', 'apply_filters' => $apply_filters]);
        $sum_accepted = $this->estimaterepo->search('', ['stats' => 'sum-accepted', 'apply_filters' => $apply_filters]);
        $sum_declined = $this->estimaterepo->search('', ['stats' => 'sum-declined', 'apply_filters' => $apply_filters]);
        $sum_expired = $this->estimaterepo->search('', ['stats' => 'sum-expired', 'apply_filters' => $apply_filters]);
        ...
        $stats = [
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-info',
                'active_panel' => 'pending',
                'url' => url('estimates?clear-filter=yes&filter_bill_status[]=new&source=stats_panels&active_panel=pending'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-success',
                'active_panel' => 'accepted',
                'url' => url('estimates?clear-filter=yes&filter_bill_status[]=accepted&source=stats_panels&active_panel=accepted'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-warning',
                'active_panel' => 'expired',
                'url' => url('estimates?clear-filter=yes&filter_bill_status[]=expired&source=stats_panels&active_panel=expired'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-danger',
                'active_panel' => 'declined',
                'url' => url('estimates?clear-filter=yes&filter_bill_status[]=declined&source=stats_panels&active_panel=declined'),
            ],
        ];
        //return
        return $stats;
