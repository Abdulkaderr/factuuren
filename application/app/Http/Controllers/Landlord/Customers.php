...
use App\Http\Responses\Landlord\Customer\ErrorLogsResponse;
...
use App\Repositories\Landlord\ErrorLogRepository;
...
class Customers extends Controller {
    ...
    public function __construct(
        ...
        //view customers
        $this->middleware('App\Http\Middleware\Landlord\Customers\View')->only([
            'index',
            'show',
            'showEmailSettings',
            'errorLogs',
        ...
        $this->middleware('App\Http\Middleware\Landlord\Customers\ViewSubscription')->only([
            'showSubscription',
        ]);

        //view customer events
        $this->middleware('App\Http\Middleware\Landlord\Customers\ViewEvents')->only([
            'events',
        ]);

        //view customer platform usage
        $this->middleware('App\Http\Middleware\Landlord\Customers\ViewPlatformUsage')->only([
            'showPlatformUsage',
        ]);

        //manage customers
        $this->middleware('App\Http\Middleware\Landlord\Customers\Manage')->only([
            'create',
            'store',
            'edit',
            'update',
            'destroy',
            'markEmailSettingsDone',
            'editPassword',
            'updatePassword',
            'setStatusActive',
            'updateStatusActive',
    ...
    public function index() {
        ...
        //get active packages for plan filter dropdown
        $packages = \App\Models\Landlord\Package::where('package_status', 'active')
    ...
    /**
     * Show the platform usage tab for a customer
     * @return \Illuminate\Http\Response
     */
    public function showPlatformUsage($id) {
        ...
        //page view
        $html = view('landlord/customer/platform-usage/page', compact('customer'))->render();
        $jsondata['dom_html'][] = [
            'selector' => '#dynamic-content-container',
            'action' => 'replace',
            'value' => $html,
    ...
    /**
     * Load the error logs tab for a customer.
     * On initial load (no log_type), renders the wrapper with the filter dropdown.
     * When a log type is selected, fetches and returns paginated log records.
     *
     * @param  \App\Repositories\Landlord\ErrorLogRepository  $errorlogrepo
     * @param  int  $id  tenant ID
     * @return \Illuminate\Http\Response
     */
    public function errorLogs(ErrorLogRepository $errorlogrepo, $id) {
        ...
        //initial tab load - no log type selected yet, render the shell wrapper
        if (!request()->filled('log_type')) {
            $html = view('landlord/customer/errorlogs/wrapper', compact('customer'))->render();
            $jsondata['dom_html'][] = [
                'selector' => '#dynamic-content-container',
                'action' => 'replace',
                'value' => $html,
        ...
        //fetch paginated logs via repository
        $logs = $errorlogrepo->search($id);
        ...
        return new ErrorLogsResponse($payload);
    }

    /**
     * Show a single error log entry in a modal.
     *
     * @param  int  $id      tenant ID
     * @param  int  $log_id  log record ID
     * @return \Illuminate\Http\Response
     */
    public function showErrorLog($id, $log_id) {

        //validate tenant exists
        if (!\App\Models\Landlord\Tenant::where('tenant_id', $id)->exists()) {
            abort(404);
        }

        //get the log record, scoped to this tenant for security
        if (!$log = \App\Models\Landlord\Log::where('log_id', $log_id)
            ->where('log_resource_id', $id)
            ->first()) {
            abort(404);
        }

        //render the log detail modal
        $html = view('landlord/customer/modals/error-log', compact('log'))->render();
        $jsondata['dom_html'][] = [
            'selector' => '#commonModalBody',
            'action' => 'replace',
            'value' => $html,
    ...
    /**
     * Delete all error log records for a customer.
     *
     * @param  int  $id  tenant ID
     * @return \Illuminate\Http\Response
     */
    public function deleteErrorLogs($id) {
        ...
        //delete all log records belonging to this tenant
        \App\Models\Landlord\Log::where('log_resource_id', $id)->delete();

        //clear the table area and hide the load more button
        $jsondata['dom_html'][] = [
            'selector' => '#error-logs-table-wrapper',
            'action' => 'replace',
            'value' => '',
        ];
        $jsondata['dom_visibility'][] = [
