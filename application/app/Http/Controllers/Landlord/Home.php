...
class Home extends Controller {
    ...
    public function index() {
        ...
        //set visibility config based on user permissions
        $this->setVisibility();
        ...
        $payload = [
            ...
            'customers' => $this->yearlyCustomers(),
        ];
    ...
    /**
     * Set visibility configs based on the authenticated user's permissions
     * @return void
     */
    private function setVisibility() {
        ...
        if (in_array($permissions->role_permissions_global_events, ['view', 'manage'])) {
            config(['visibility.home_events_panel' => true]);
        }

        if (in_array($permissions->role_permissions_payments, ['view', 'manage'])) {
            config(['visibility.home_payments_panel' => true]);
        }
    }
    ...
    /**
     * yearly customer signups graph
     *
     * @return array
     */
    public function yearlyCustomers() {
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $count = \App\Models\Landlord\Tenant::where('tenant_created', '>=', $start_date)
                ->where('tenant_created', '<=', $end_date)
                ->count();
