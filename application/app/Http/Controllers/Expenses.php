...
use App\Http\Responses\Expenses\EditTagsResponse;
...
class Expenses extends Controller {
    ...
    public function __construct(
        ...
        //Filtering
        $this->middleware('expensesMiddlewareFiltering')->only([
            'index',
        ]);
    ...
    public function index(ProjectRepository $projectrepo, CategoryRepository $categoryrepo) {
        ...
        $tags = $this->tagrepo->getByType('expense');
    ...
    public function store(ExpenseStoreUpdate $request) {
        ...
        //fire storing event
        event(new \App\Events\Expenses\ExpenseStoring(request()));
        ...
        //add tags
        $this->tagrepo->add('expense', $expense_id);
        ...
        //fire stored event
        event(new \App\Events\Expenses\ExpenseStored(request(), $expense_id, $payload));
    ...
    public function update($id) {
        ...
        //fire updating event
        event(new \App\Events\Expenses\ExpenseUpdating(request(), $id));
        ...
        //fire updated event
        event(new \App\Events\Expenses\ExpenseUpdated(request(), $id, $payload));
    ...
    public function destroy(DestroyRepository $destroyrepo) {
        ...
        //fire deleting event
        event(new \App\Events\Expenses\ExpenseDeleting(request(), array_keys(array_filter(request('ids'), function ($value) {
            return $value == 'on';
        }))));
        ...
        //fire deleted event
        event(new \App\Events\Expenses\ExpenseDeleted(request(), $allrows, $payload));
    ...
    public function downloadAttachment() {
        ...
        if ($attachment->attachment_filename != '') {
            ...
            if (Storage::exists($file_path)) {
                //fire attachment downloaded event
                event(new \App\Events\Expenses\ExpenseAttachmentDownloaded(request(), request()->route('uniqueid')));
    ...
    public function attachDettachUpdate(ExpenseAttachProject $request) {
        ...
        event(new \App\Events\Expenses\ExpenseProjectAttached(request(), request()->route('expense'), $payload));
    ...
    public function deleteAttachment() {
        ...
        //fire attachment deleting event
        event(new \App\Events\Expenses\ExpenseAttachmentDeleting(request(), request()->route('uniqueid')));
        ...
        //fire attachment deleted event
        event(new \App\Events\Expenses\ExpenseAttachmentDeleted(request(), request()->route('uniqueid')));
    ...
    public function changeCategoryUpdate(CategoryRepository $categoryrepo) {
        ...
        event(new \App\Events\Expenses\ExpenseCategoryChanged(request(), array_keys(array_filter(request('ids'), function ($value) {
            return $value == 'on';
        })), $payload));
    ...
    public function togglePinning(PinnedRepository $pinrepo, $id) {
        ...
        event(new \App\Events\Expenses\ExpensePinToggled(request(), $id, $payload));
    ...
    public function recurringSettingsUpdate(ExpenseRecurrringSettings $request, $id) {
        ...
        //fire recurring settings updating event
        event(new \App\Events\Expenses\ExpenseRecurringSettingsUpdating(request(), $id));
        ...
        //fire recurring settings updated event
        event(new \App\Events\Expenses\ExpenseRecurringSettingsUpdated(request(), $id, $payload));
    ...
    public function stopRecurring() {
        ...
        event(new \App\Events\Expenses\ExpenseRecurringStopped(request(), request()->route('expense'), $payload));
 ...
 */
    public function storeClone($id) {
        ...
        //fire cloning event
        event(new \App\Events\Expenses\ExpenseCloning(request(), $id));
        ...
        if (request('clone_files') == 'on') {
            ...
            foreach ($attachments as $attachment) {
                //paths
                $old_directory = BASE_DIR . "/storage/files/" . $attachment->attachment_directory;
                ...
                if (is_dir($old_directory)) {
                    ...
                    $new_path = BASE_DIR . "/storage/files/" . $new_directory;
                    //copy directory
        ...
        //fire cloned event
        event(new \App\Events\Expenses\ExpenseCloned(request(), $id, $new_expense->expense_id, $payload));
    ...
    /**
     * Update the tags for the specified expense
     * @param int $id expense id
     * @return \Illuminate\Http\Response
     */
    public function updateTags($id) {
        ...
        $this->tagrepo->delete('expense', $id);

        //add new tags
        $this->tagrepo->add('expense', $id);
