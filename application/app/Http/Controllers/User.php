...
use App\Http\Responses\User\CardsViewModeReposnse;
...
class User extends Controller {
    ...
    /**
     * Update the authenticated user's distraction free mode preference
     * @return \Illuminate\Http\Response
     */
    public function updateDistractionFreeMode() {

        $action = request('action');

        //set preference based on action
        $pref_value = ($action == 'enable') ? 'yes' : 'no';

        //save to database
        auth()->user()->pref_view_kanban_card_distraction_free = $pref_value;
        auth()->user()->save();
        ...
        return new CardsViewModeReposnse($payload);
    }
