...
use App\Http\Requests\Invoices\CancelInvoice;
use App\Http\Requests\Invoices\ChangeClient;
...
use App\Http\Responses\Invoices\CancelInvoiceResponse;
use App\Http\Responses\Invoices\ChangeCategoryUpdateResponse;
use App\Http\Responses\Invoices\ChangeClientResponse;
...
use App\Http\Responses\Invoices\ShowPublicResponse;
...
use App\Http\Responses\Invoices\EditTagsResponse;
use App\Http\Responses\Invoices\UpdateTaxtypeResponse;
use App\Http\Responses\Pay\FlutterwavePaymentResponse;
...
use App\Repositories\FlutterwavePaymentRepository;
...
class Invoices extends Controller {
    ...
    public function __construct(
        ...
        //public view
        $this->middleware('invoicesMiddlewareShowPublic')->only([
            'showPublic',
        ]);
    ...
    /**
     * Display invoice via public URL (no authentication required)
     * @param string $id the invoice unique id (bill_uniqueid)
     * @return \Illuminate\Http\Response
     */
    public function showPublic($id) {

        $invoice = \App\Models\Invoice::Where('bill_uniqueid', $id)->first();
    ...
    /**
     * show the form for changing the invoice client
     * @param int $id invoice id
     * @return \Illuminate\Http\Response
     */
    public function changeClient($id) {
        ...
        //show the form
        return new ChangeClientResponse($payload);
    }

    /**
     * update the client for an invoice
     * @param object ChangeClient instance of the request validation
     * @param int $id invoice id
     * @return \Illuminate\Http\Response
     */
    public function changeClientAction(ChangeClient $request, $id) {
        ...
        //validation: invoice status (4 = part paid, 5 = paid)
        if (in_array($invoice->bill_status, [4, 5])) {
            abort(409, __('lang.invoice_already_paid'));
        }

        //validation: invoice type must be onetime
        if ($invoice->bill_invoice_type != 'onetime') {
            abort(409, __('lang.action_not_allowed_for_subscription_invoices'));
        }

        //validation: must be an invoice (not estimate)
        if ($invoice->bill_type != 'invoice') {
            abort(409, __('lang.error_request_could_not_be_completed'));
        }

        //validation: recurring invoices with issued children
        if ($invoice->bill_recurring_last != null) {
            abort(409, __('lang.action_not_allowed_for_recurring_invoices'));
        }

        //validation: subscription invoices
        if ($invoice->bill_subscriptionid != null) {
            abort(409, __('lang.action_not_allowed_for_subscription_invoices'));
        }
    ...
    public function paymentStripe(StripePaymentRepository $striperepo, InvoiceRepository $invoicerepo, $id) {
        ...
        //get payee details (authenticated user or primary account owner for guests)
        $payee = $this->setPayeeDetails($invoice);
    ...
    public function paymentRazorpay(RazorpayPaymentRepository $razorpayrepo, InvoiceRepository $invoicerepo, $id) {
        ...
        $payload['client_email'] = $payee['email'];
     ...
     * @param int $id client id
     */
    public function paymentFlutterwave(FlutterwavePaymentRepository $flutterwaverepo, InvoiceRepository $invoicerepo, $id) {
        ...
        $data = [
            'amount' => $invoice->invoice_balance,
            'currency' => strtoupper(config('system.settings2_flutterwave_currency_code')),
            'item_name' => __('lang.invoice_payment'),
            'invoice_id' => $invoice->bill_invoiceid,
            'email' => $payee['email'],
            'thank_you_url' => url('payments/thankyou'),
        ...
        //create a new flutterwave session
        if (!$checkout_url = $flutterwaverepo->onetimePayment($data)) {
            abort(409, __('lang.error_request_could_not_be_completed'));
        }
        ...
        //show the view
        return new FlutterwavePaymentResponse($payload);
    }
    ...
    /**
     * Get payee details for payment processing
     * Returns authenticated user details or primary account owner details for guest
     * @param object $invoice The invoice object
     * @return array payee details array
     */
    private function setPayeeDetails($invoice) {
        ...
        if (auth()->check()) {
            ...
            //guest mode - get primary account owner from client
            $client = \App\Models\Client::Where('client_id', $invoice->bill_clientid)->first();

            if ($client && $client->primary_user) {
                ...
                $payee = [
                    ...
                    'clientid' => $primary_user->clientid,
                ];
            }
        }
    ...
    private function statsWidget($data = array()) {
        ...
        //when a stats panel is clicked, the URL carries filter params (e.g. bill status) that scope the list results.
        //we must not let those params also affect the stats queries, or every panel would reflect the filtered totals
        //instead of its own independent value. disabling apply_filters here ensures stats always show global figures.
        $apply_filters = (request('source') == 'stats_panels') ? false : true;

        //stats
        $count_all = $this->invoicerepo->search('', ['stats' => 'count-all', 'apply_filters' => $apply_filters]);
        $count_due = $this->invoicerepo->search('', ['stats' => 'count-due', 'apply_filters' => $apply_filters]);
        $count_overdue = $this->invoicerepo->search('', ['stats' => 'count-overdue', 'apply_filters' => $apply_filters]);

        $sum_all = $this->invoicerepo->search('', ['stats' => 'sum-all', 'apply_filters' => $apply_filters]);
        $sum_paid = $this->invoicerepo->search('', ['stats' => 'sum-paid', 'apply_filters' => $apply_filters]);
        $count_paid = $this->invoicerepo->search('', ['stats' => 'count-paid', 'apply_filters' => $apply_filters]);
        $sum_due_balances = $this->invoicerepo->search('', ['stats' => 'sum-due-balances', 'apply_filters' => $apply_filters]);
        $sum_overdue_balances = $this->invoicerepo->search('', ['stats' => 'sum-overdue-balances', 'apply_filters' => $apply_filters]);
        ...
        $stats = [
            [
                ...
                'color' => 'bg-info',
                'active_panel' => 'all',
                'url' => url('invoices?clear-filter=yes&source=stats_panels&active_panel=all'),
            ],
            [
                'value' => runtimeMoneyFormat($sum_paid),
                'title' => __('lang.paid') . " ($count_paid)",
                'percentage' => '100%',
                'color' => 'bg-success',
                'active_panel' => 'paid',
                'url' => url('invoices?clear-filter=yes&filter_bill_status[]=5&source=stats_panels&active_panel=paid'),
            ],
            [
                ...
                'color' => 'bg-warning',
                'active_panel' => 'due',
                'url' => url('invoices?clear-filter=yes&filter_bill_status[]=2&source=stats_panels&active_panel=due'),
            ],
            [
                ...
                'percentage' => '100%',
                'color' => 'bg-danger',
                'active_panel' => 'overdue',
                'url' => url('invoices?clear-filter=yes&filter_bill_status[]=3&source=stats_panels&active_panel=overdue'),
            ],
        ];
        //return
        return $stats;
    ...
    /**
     * Show the bulk change status form
     * @return \Illuminate\Http\Response
     */
        ...
        foreach (request('ids') as $bill_invoiceid => $value) {
            if ($value == 'on') {
                if ($invoice = \App\Models\Invoice::Where('bill_invoiceid', $bill_invoiceid)->first()) {
                    ...
                    //get refreshed invoice
                    $invoices = $this->invoicerepo->search($bill_invoiceid, ['apply_filters' => false]);
    ...
    /**
     * Bulk send overdue reminders to clients
     * @return \Illuminate\Http\Response
     */
    public function bulkOverdueReminder() {
        ...
        foreach (request('ids') as $bill_invoiceid => $value) {
            if ($value == 'on') {
                ...
                //skip invoices that are not overdue
                if ($invoice->bill_status != 3) {
                    continue;
                }
                ...
                $reminder_count = $invoice->bill_overdue_reminder_counter + 1;
                \App\Models\Invoice::Where('bill_invoiceid', $bill_invoiceid)
                    ->update([
                        'bill_overdue_reminder_counter' => $reminder_count,
                        'bill_overdue_reminder_last_sent' => now(),
                    ]);
        ...
        $payload = [
            'allrows' => $allrows,
            'response' => 'send-overdue-reminder',
        ];
    ...
    public function cancelCreate($id) {
        ...
        return new CancelInvoiceResponse($payload);
    }

    /**
     * Process the invoice cancellation
     * @param int $id invoice id
     */
    public function cancelStore(CancelInvoice $request, $id) {
        ...
        $invoice->bill_status = 20;
        $invoice->bill_cancellation_date = \Carbon\Carbon::now();
        $invoice->bill_cancellation_userid = auth()->id();
        $invoice->bill_cancellation_reasons = request('html_bill_cancellation_reasons');
        $invoice->bill_status_trigger = 'manual';
        $invoice->save();

        //refund all payments if requested (ajax framework sends 'on' for checked checkboxes)
        if (request('refund_payments') == 'on') {
            foreach ($invoice->payments as $payment) {
                //skip payments that already have a refund
                ...
                $refund->refund_paymentid = $payment->payment_id;
                $refund->refund_clientid = $payment->payment_clientid;
                $refund->refund_invoiceid = $payment->payment_invoiceid;
                $refund->refund_creatorid = auth()->id();
                $refund->refund_amount = $payment->payment_amount;
                $refund->refund_notes = $invoice->bill_cancellation_reasons;
                $refund->refund_date = \Carbon\Carbon::now()->format('Y-m-d');
                $refund->save();
        ...
        $payload = ['response' => 'update-status'];
        return new ChangeStatusResponse($payload);
    }
    ...
    public function restoreInvoice($id) {
        ...
        $invoice->bill_cancellation_date = null;
        $invoice->bill_cancellation_userid = null;
        $invoice->bill_cancellation_reasons = null;
        $invoice->bill_status_trigger = 'automatic';
        $invoice->save();
    ...
    public function editTags($id) {
        ...
        //get tags
        $tags_resource = $this->tagrepo->getByResource('invoice', $id);
        $tags_system = $this->tagrepo->getByType('invoice');
    ...
    public function updateTags($id) {
        ...
        //get all tags (type: invoice) - for filter panel
        $tags = $this->tagrepo->getByType('invoice');
