...
class CreateAccount extends FormRequest {
    ...
    public function messages() {
        return [
            ...
            'country.required' => __('lang.country') . ' - ' . __('lang.is_required'),
            'telephone.required' => __('lang.telephone') . ' - ' . __('lang.is_required'),
            'telephone.min' => __('lang.telephone') . ' - ' . __('lang.is_invalid'),
            'telephone.max' => __('lang.telephone') . ' - ' . __('lang.is_invalid'),
            'telephone.regex' => __('lang.telephone') . ' - ' . __('lang.is_invalid'),
        ];
    }
    ...
    public function rules() {
        ...
        //country validation (only when set to required in signup form settings)
        if (config('system.settings_signup_form_country') == 'required') {
            $rules += [
                'country' => ['required'],
            ];
        }

        //telephone validation (only when set to required in signup form settings)
        if (config('system.settings_signup_form_telephone') == 'required') {
            $rules += [
                'telephone' => ['required', 'min:7', 'max:20', 'regex:/^[0-9\s\+\-\(\)\.]+$/'],
            ];
        }
