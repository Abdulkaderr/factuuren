 ...
 */
/**
 * Format a monetary value using the tenant's currency settings.
 * Falls back to US dollar formatting ($, left, fullstop decimal, comma thousands)
 * if any setting is not available on the customer object.
 *
 * @param  float        $amount    The raw monetary amount
 * @param  object|null  $customer  The landlord Tenant model instance
 * @return string  Fully formatted monetary string e.g. "$1,234.56" or "1.234,56 €"
 */
function tenantMoneyFormat($amount = 0, $customer = null) {

    $symbol   = ($customer->tenant_settings_currency_symbol ?? null) ?: '$';
    $position = ($customer->tenant_settings_currency_position ?? null) ?: 'left';
    $decimal  = ($customer->tenant_settings_decimal_separator ?? null) ?: 'fullstop';
    $thousand = ($customer->tenant_settings_thousand_separator ?? null) ?: 'comma';

    // Map setting names to actual characters
    $decimal_char = ($decimal === 'comma') ? ',' : '.';
    ...
    $thousand_char = $thousand_map[$thousand] ?? ',';

    $formatted = number_format((float) $amount, 2, $decimal_char, $thousand_char);
